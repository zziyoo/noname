...
const translates = {
	...
	sbzhejian: "谪奸",
	sbzhejian_info: "限定技，出牌阶段，你可弃置所有基本牌并选择一名角色。若如此做，每回合首次有角色连续使用牌指定其为目标后，使用者需选择一项：1.对其造成一点伤害；2.令1号位摸两张牌。直到其因此进入濒死状态。",
	sp_yangwan: "谋杨婉",
	spmingxuan: "瞑昡",
	...
	sbjieyin: "结姻",
	sbjieyin_info: `使命技，你的登场势力为蜀，游戏开始时，你将一名其他角色选择为结姻角色。结姻角色死亡后，你减一点体力上限；结姻角色的出牌阶段开始时，你选择一项：1.令其获得一点护甲；2.你将势力变更为吴，然后获得其X张牌（X为游戏轮数且至多为3）。成功：若结姻角色护甲值为5，你每轮首次发动${get.poptip("xinsbxiaoji")}后，获得1点护甲。成功：若结姻角色拥有4个装备，你发动${get.poptip("xinsbxiaoji")}后，本回合使用【杀】的次数+1。失败：若结姻角色死亡或你的势力为吴，你清除结姻标记，重铸所有红桃手牌，然后摸两张牌。`,
	sbliangzhu: "良助",
	sbliangzhu_info: "蜀势力技，锁定技。你或结姻角色回复体力后，对方摸一张牌。出牌阶段结束时，若你已受伤，你回复1点体力；否则你观看三张装备牌，然后选择交给一名结姻角色（若其对应装备栏为空置则其使用之）。",
	sbxiaoji: "枭姬",
	sbxiaoji_info: "你失去装备区里的一张牌后，你摸两张牌。若你：有失败的使命技，你执行摸牌后可弃置场上一张牌；有成功的使命技，结姻角色失去装备时也可触发。",
	sb_sunquan: "谋孙权",
	sbzhiheng: "制衡",
	sbzhiheng_info: "出牌阶段限一次，你可弃置任意张牌并摸等量+X张牌（X为你因此弃置牌后牌数相等的区域数）。",
	sbtongye: "统业",
	...
	sb_zhanghe: "谋张郃",
	sbqiaobian: "巧变",
	sbqiaobian_info: "准备阶段，你可以弃置任意区域里的各一张牌，跳过等量个阶段（判定阶段和结束阶段除外），并于跳过对应阶段后执行对应效果。若以此法跳过：摸牌阶段，你可以获得至多两名其他角色各一张手牌；出牌阶段，你可以移动场上的一张牌，然后若你的牌数未增加，你可以令牌数因此增加的角色摸两张牌；弃牌阶段，你摸一张牌，然后结束阶段，若你的手牌数不小于场上角色数，你执行一个额外的出牌阶段。（你因此获得的牌或移动至你装备区的牌称为“巧变”牌）",
	sbliaoshi: "料势",
	//sbliaoshi_info: `每回合限一次，你可以将一张因${get.poptip("sbqiaobian")}获得的牌当任意基本牌使用或打出。`,
	...
	sbweiliu: "卫旒",
	sbweiliu_info: "每轮每项限一次，有“任”的角色回合开始时，你可以选择一项执行：1.弃置X张与“任”颜色相同的牌，对一名角色造成一点伤害；2.从牌堆获得X张与“任”颜色相同的牌。（X为你本局游戏“明任”的发动次数）该角色的回合结束时，若其本回合达成了你未执行的一项，你将其“任”置入弃牌堆。",
	old_sbmingren: "明任",
