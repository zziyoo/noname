import { _status, ui, game, lib, get } from "noname";
const type = "card";
const guozhan = {
  name: "guozhan",
  connect: true,
  card: {
    ...
    gz_haolingtianxia: {
      ...
      async content(event, trigger, player) {
        ...
        const targets = game.filterPlayer((current) => current !== target).sortBySeat();
        for (const current of targets) {
          ...
          if (choices.length > 1) {
            const { control } = await current.chooseControl(choices).set("prompt", "号令天下：请选择其中一项").set("target", target).set("ai", () => {
              const player2 = get.event().player;
              const target2 = get.event().target;
              const choices2 = get.event().controls;
              const guohe = new lib.element.VCard({ name: "guohe_copy2" });
              const shunshou = new lib.element.VCard({ name: "shunshou_copy2" });
              const sha = new lib.element.VCard({ name: "sha" });
              const num = Math.max(0, choices2.includes("弃牌") ? get.effect(target2, guohe, player2) : 0, choices2.includes("得牌") ? get.effect(target2, shunshou, player2) : 0);
              if (choices2.includes("出杀") && get.effect(player2, guohe, player2) + get.effect(target2, sha, player2) > num) {
                return "出杀";
              }
              if (choices2.includes("得牌") && num > 0) {
                return "得牌";
              }
              if (choices2.includes("弃牌") && num > 0) {
                return "弃牌";
              }
              return "cancel2";
            }).forResult();
    ...
    gz_guguoanbang: {
      ...
      async content(event, trigger, player) {
        ...
        const result = await target.chooseToDiscard("请弃置至少六张手牌", [6, target.countCards("h")], true, "h", "allowChooseAll").set("ai", (card) => {
          const player2 = get.event().player;
          const targetx = get.event().targetx;
          if (2 * ui.selected.targets >= targetx.filter((current) => get.attitude(player2, current) > 0).length) {
            return 0;
          }
        ...
        while (targets.length && give_cards.length) {
          let result2;
          result2 = await target.chooseButton(["是否将弃置的牌交给其他吴势力角色？", give_cards], [1, 2]).set("ai", (button) => {
            const player2 = get.event().player;
            const targetx = get.event().targetx;
            const { link } = button;
            if (targetx.some((current) => get.attitude(player2, current) > 0)) {
              return get.value(link);
            }
            if (!ui.selected.buttons.length && get.name(link) === "du" && targetx.some((current) => get.attitude(player2, current) < 0 && !current.hasSkillTag("nodu"))) {
              return 1;
            }
            return 0;
          }).set("targetx", targets).set("cards", give_cards).forResult();
          if (result2?.bool && result2?.links?.length) {
            const { links } = result2;
            result2 = await target.chooseTarget(true, `选择获得${get.translation(links)}的角色`, (card, player2, target2) => {
              return get.event().targetx.includes(target2);
            }).set("targetx", targets).set("ai", (target2) => {
              const player2 = get.event().player;
              const toEnemy = get.event().toEnemy;
              ...
              return Math.max(0.1, att / Math.sqrt(1 + target2.countCards("h")));
            }).set("toEnemy", get.name(links[0]) === "du").forResult();
            if (result2?.bool && result2?.targets?.length) {
              const [current] = result2.targets;
    ...
    gz_wenheluanwu: {
      ...
      async content(event, trigger, player) {
        ...
        const str = get.translation(target);
        const result = await player.chooseControl().set("prompt", "文和乱武：请选择一项").set("choiceList", [`令${str}弃置两张类型不同的手牌`, `弃置${str}的一张手牌`]).set("ai", () => {
          ...
          hs.forEach((i) => {
            type2.add(get.type2(i, target2));
          });
          if (target2[judge] !== "qun") {
            if (Boolean(att > 0) === Boolean(type2.length > 1)) {
              return 1;
            }
            return 0;
          }
          if (type2.length < 2 || target2.hp < 3) {
            return att > 0 ? 1 : 0;
          }
          if (hs.length === 2) {
            return att > 0 ? 0 : 1;
        ...
        if (result.index === 0) {
          ...
          if (list.length > 1) {
            result2 = await target.chooseToDiscard("h", true, "请弃置两张类型不同的手牌", 2, (card, player2) => {
    ...
    liulongcanjia: {
      ...
      ai: {
        equipValue(card, player) {
          ...
          if (game.hasPlayer((current) => current.hasSkill("gzzongyu") && get.attitude(player, current) <= 0)) {
            return 1;
          }
          return 7.2;
        },
    ...
    dinglanyemingzhu: {
      ...
      ai: {
        equipValue(card, player) {
          ...
          if (game.hasPlayer((current) => current.hasSkill("jubao") && get.attitude(player, current) <= 0)) {
            return 0;
          }
          return 7;
        },
    ...
    feilongduofeng: {
      ...
      ai: {
        equipValue(card, player) {
          ...
          if (game.hasPlayer((current) => current.hasSkill("zhangwu") && get.attitude(player, current) <= 0)) {
            return 1;
          }
          return 8;
        },
    ...
    taipingyaoshu: {
      ...
      ai: {
        equipValue(card, player) {
          ...
          if (game.hasPlayer((current) => current.hasSkill("wendao") && get.attitude(player, current) <= 0)) {
            return 1;
          }
          return 6;
        },
    ...
    jilinqianyi: {
      ...
      ai: {
        equipValue(card, player) {
          ...
          if (game.hasPlayer((current) => current.hasSkill("gz_shujuan") && get.attitude(player, current) <= 0 && player.getDamagedHp())) {
            return 1;
          }
          return 4;
        },
    ...
    xietianzi: {
      ...
      async content(event, trigger, player) {
        ...
        event.target.addTempSkill("xietianzi");
      },
      ai: {
        order: 0.5,
        value: 4,
    ...
    shuiyanqijunx: {
      ...
      cardnature: "thunder",
      filterTarget(card, player, target) {
        return target !== player && (get.mode() !== "guozhan" || _status.mode === "yingbian" || _status.mode === "free" || target.hasCards("e"));
      },
      enable: true,
      defaultYingbianEffect: "add",
      async content(event, trigger, player) {
        const target = event.target;
        const discardableEquips = target.getCards("e", (card) => lib.filter.cardDiscardable(card, target, "shuiyanqijunx"));
        if (event.card.yingbian_all) {
          await target.discard({ cards: discardableEquips });
          await target.damage({ nature: "thunder" });
          return;
        }
        if (!discardableEquips.length) {
        ...
        const result = await target.chooseControl({
          prompt: "水淹七军",
          prompt2: `请选择一项：⒈弃置装备区里的所有牌；⒉受到${get.translation(player)}造成的1点雷电伤害。`,
          controls: ["discard_card", "take_damage"],
          ai(event2, player2) {
            const eff = get.damageEffect(player2, event2.player, player2, "thunder");
            if (eff > 0) {
              return "take_damage";
            }
            ...
            if (player2.isDamaged() && player2.hasCard((card) => get.name(card) === "baiyin" && get.recoverEffect(player2, player2, _status.event.player) > 0, "e")) {
              return "discard_card";
            }
            if (player2.hasCard((card) => get.value(card, player2) <= 0, "e") && !player2.hasCard((card) => get.value(card, player2) > Math.max(7, 12 - player2.hp), "e")) {
              return "discard_card";
            }
            ...
            if (player2.hp > 2 && player2.countCards("e") > 2 || player2.hp > 1 && player2.countCards("e") > 3) {
              return "take_damage";
            }
            return "discard_card";
          }
      ...
      ai: {
        ...
        yingbian(card, player, targets, viewer) {
          ...
          if (get.cardtag(card, "yingbian_all")) {
            if (targets.filter((current) => {
              return get.damageEffect(current, player, player, "thunder") > 0 && current.countCards("e", (card2) => get.value(card2, current) <= 0) < 2 && current.countCards("e", (card2) => get.value(card2, current) > 0) > 0;
            }).length) ;
          }
        ...
        result: {
          target(player, target, card, isLink) {
            const es = target.getCards("e");
            const eff = 2 * get.sgn(get.damageEffect(target, player, target, "thunder"));
            if (isLink || !es.length) {
              return eff;
            }
            let val = 0;
    ...
    lulitongxin: {
      ...
      enable(card, player) {
        ...
        return game.hasPlayer((current) => current.isMajor());
      },
      mode: ["guozhan", "versus"],
      filterTarget: true,
      recastable: true,
      ...
      changeTarget(player, targets) {
        if (get.mode() === "guozhan") {
          ...
          if (target.identity !== "ye") {
            game.filterPlayer((current) => target !== current && target.isFriendOf(current) && !current.hasSkill("diaohulishan"), targets);
          }
        }
      },
      async content(event, trigger, player) {
        ...
        if (get.mode() !== "guozhan") {
          if (player === target) {
            await target.draw(game.filterPlayer().length);
          } else {
            await target.chooseDrawRecover(true);
          }
          return;
        }
        if (target !== player) {
        ...
        if (typeof result.control === "string") {
          ...
          if (recover) {
            await target.recover({ num: recover });
          }
        }
      },
      ai: {
    ...
    chiling: {
      ...
      type: "trick",
      enable() {
        return game.hasPlayer((current) => current.isUnseen());
      },
      mode: ["guozhan"],
      ...
      async content(event, trigger, player) {
        const target = event.target;
        const choiceList = ["明置一张武将牌，然后摸一张牌", "失去1点体力"];
        event.nomingzhi = target.hasSkillTag("nomingzhi", false, null, true);
        if (event.nomingzhi) {
          choiceList.shift();
        }
        if (target.hasCards("he", { type: "equip" })) {
          choiceList.push("弃置一张装备牌");
        }
        const result = await target.chooseControl({
          prompt: "敕令",
          choiceList,
          ai: lib.card.chiling.chooseai
        }).forResult();
      ...
      onDestroy() {
        ...
        if (!lib.inpile.includes("zhaoshu")) {
          ...
          const card = game.createCard2("zhaoshu", "club", 3);
    ...
    diaohulishan: {
      ...
      selectTarget: [1, 2],
      async content(event, trigger, player) {
        event.target.addTempSkill("diaohulishan");
      },
      ai: {
        order: 10,
        value: 4,
    ...
    huoshaolianying: {
      ...
      filterTarget(card, player, target) {
        ...
        if (game.hasPlayer((current) => current.isLinked() && current !== player)) {
          if (!target.isLinked()) {
            return false;
          }
          const distance = get.distance(player, target, "absolute");
          return !game.hasPlayer((current) => {
            if (target !== current && current !== player && current.isLinked()) {
              const dist2 = get.distance(player, current, "absolute");
              if (dist2 < distance) {
                return true;
              }
              if (dist2 === distance && parseInt(current.dataset.position) < parseInt(target.dataset.position)) {
                return true;
              }
            }
          });
      ...
      ai: {
        ...
        result: {
          target(player, target, card) {
            if (target.hasSkillTag("nofire") || target.hasSkillTag("nodamage", null, {
              source: player,
              card,
              natures: ["fire"]
            })) {
    ...
    zhibi: {
      ...
      filterTarget(card, player, target) {
        ...
        return target.isUnseen(2) || target.hasCards("h");
      },
      async content(event, trigger, player) {
        const target = event.target;
        ...
        await player.chooseControl({ controls: ["ok"] }).set("dialog", content);
      },
      mode: ["guozhan"],
      ai: {
        order: 9.5,
    ...
    yiyi: {
      ...
      async content(event, trigger, player) {
        ...
        event.target.chooseToDiscard(2, "he", true).ai = get.disvalue;
      },
      ai: {
        wuxie() {
          return 0;
    ...
    wuliu: {
      ...
      ai: {
        equipValue(card, player) {
          ...
          return 2 + game.countPlayer((current) => current.isFriendOf(player)) / 2;
        },
        basic: {
          equipValue: 3
        }
  ...
  skill: {
    zhaoshu_skill: {
      ...
      usable: 1,
      filter(event, player) {
        const cards = player.getExpansions("zhaoshu_cards");
        if (cards.length < 4) {
          return false;
        }
        const suits = new Set(cards.map((card) => get.suit(card, false)));
        return suits.size >= 4;
      },
      delay: false,
      async content(event, trigger, player) {
        ...
        ].filter((card) => !lib.inpile.includes(card[2]));
        if (list.length) {
      ...
      intro: {
        ...
        mark(dialog, content, player) {
          const expansionCards = player.getExpansions("zhaoshu_skill");
          dialog.add(expansionCards);
          dialog.addText("<ul><li>与你势力相同的角色的出牌阶段限一次，其可以将一张手牌（小势力角色改为至多两张）置于【诏书】上，称为“应”。</li><li>出牌阶段限一次，若你的“应”中包含至少四种花色，则你可以发动“锦囊召唤”，将所有“应”置入弃牌堆，然后随机获得一张未加入游戏的势力锦囊牌。</li></ul>", false);
          const cards = player.getExpansions("zhaoshu_cards");
          if (cards.length) {
      ...
      onremove(player, skill) {
        const cards = player.getExpansions(skill).concat(player.getExpansions("zhaoshu_cards"));
        if (cards.length) {
    ...
    zhaoshu_global: {
      ...
      filter(event, player) {
        ...
        return game.hasPlayer((current) => current.hasSkill("zhaoshu_skill") && current.isFriendOf(player));
      },
      filterCard: true,
      selectCard() {
      ...
      check(card) {
        ...
        const cards = ui.selected.cards.concat(game.findPlayer((current) => current.hasSkill("zhaoshu_skill") && current.isFriendOf(player))?.getExpansions("zhaoshu_cards") ?? []);
        const suit = get.suit(card, false);
        for (const current of cards) {
          if (get.suit(current) === suit) {
            return 0;
          }
        }
      ...
      selectTarget() {
        if (game.countPlayer((current) => current.hasSkill("zhaoshu_skill") && current.isFriendOf(_status.event.player)) === 1) {
          return -1;
        }
        return 1;
      },
      prompt() {
        const player = _status.event.player;
        const targets = game.filterPlayer((current) => current.hasSkill("zhaoshu_skill") && current.isFriendOf(player));
        return `将${player.isNotMajor() ? "至多两" : "一"}张手牌置于${get.translation(targets)}的【诏书】上`;
      },
      async content(event, trigger, player) {
        const target = event.target;
    ...
    dinglanyemingzhu_skill: {
      ...
      filterCard(card, player) {
        const cards = player.getEquips("dinglanyemingzhu");
        if (cards.length) {
      ...
      ai: {
        effect: {
          player_use(card, player) {
            ...
            if (card.name === "dinglanyemingzhu" && game.hasPlayer((current) => current.hasSkill("jubao") && get.attitude(player, current) <= 0)) {
              return [0, 0, 0, 0];
            }
          }
        }
      ...
      ai: {
        effect: {
          player_use(card, player) {
            ...
            if (card.name === "feilongduofeng" && game.hasPlayer((current) => current.hasSkill("zhangwu") && get.attitude(player, current) <= 0)) {
              return [0, 0, 0, 0];
            }
          }
        }
      ...
      ai: {
        effect: {
          player_use(card, player) {
            ...
            if (card.name === "taipingyaoshu" && game.hasPlayer((current) => current.hasSkill("wendao") && get.attitude(player, current) <= 0)) {
              return [0, 0, 0, 0];
            }
          },
          target_use(card, player, target) {
            ...
            if (get.subtype(card) === "equip2" && target.getEquip("taipingyaoshu") && !target.countEmptySlot(2)) {
              ...
              const lose = get.effect(target, { name: "losehp" }, target, target);
              let draw = 2 * get.effect(target, { name: "draw" }, target, target);
        ...
        async (event, trigger, player) => {
          ...
          if (trigger.player === game.me && !_status.auto) {
            ...
            trigger.player.send(
              (player2, list2) => {
                if (_status.auto) {
                  _status.event._result = list2.randomGet();
                } else {
                  const next = game.createEvent("replacePlayer");
                  next.source = player2;
                  next.list = list2;
                  next.setContent(() => {
                    ...
                    event.switchToAuto = () => {
                      event.result = event.list.randomGet();
                      event.dialog.close();
                      if (ui.confirm) {
                        ui.confirm.close();
                      }
    ...
    feilongduofeng3: {
      ...
      filter(event, player) {
        const evt = event.getParent("damage");
      ...
      mod: {
        maxHandcard(player, num) {
          if (get.mode() === "guozhan") {
            ...
            return num + game.countPlayer((current) => current.isFriendOf(player));
          }
    ...
    yuxi_skill2: {
      ...
      filter(event, player) {
        ...
        return game.hasPlayer((current) => player.canUse("zhibi", current));
      },
      async content(event, trigger, player) {
    ...
    g_chiling3: {
      ...
      async content(event, trigger, player) {
        ...
        const targets = game.filterPlayer((target) => target.isUnseen());
    ...
    huxinjing: {
      ...
      filter(event, player) {
        ...
        const cards = player.getEquips("huxinjing");
        if (!cards.length) {
          return false;
        }
        if (player.hasSkillTag("unequip2")) {
      ...
      async content(event, trigger, player) {
        trigger.cancel();
        const e2 = player.getEquips("huxinjing");
        if (e2.length) {
          player.discard(e2);
        }
    ...
    g_wuliu_skill: {
      ...
      mod: {
        attackRange(player, distance) {
          return distance + game.countPlayer((current) => {
    ...
    sanjian_skill: {
      ...
      filter(event, player) {
        ...
        return game.hasPlayer((current) => current !== event.player && get.distance(event.player, current) <= 1);
      },
      async content(event, trigger, player) {
        ...
        const result = await player.chooseCardTarget({
          ...
          filterTarget(card, player2, target) {
            const damaged2 = _status.event.damaged;
            return get.distance(damaged2, target) <= 1 && target !== damaged2;
          },
          ai1(card) {
    ...
    jilinqianyi_skill: {
      ...
      mod: {
        attackRange(player, distance) {
          const cards = player.getEquips("jilinqianyi");
