      ...
      ai: {
        ...
        yingbian(card, player, targets, viewer) {
          ...
          if (get.cardtag(card, "yingbian_all")) {
            if (targets.some((current) => {
              ...
              if (att <= 0) {
                return current.countCards("he", (card2) => {
                  return get.value(card2, current) > 0;
                }) > 1;
              }
              return current.countCards("ej", (card2) => {
        ...
        result: {
          ...
          player(player, target) {
            ...
            if (get.attitude(player, target) < 0 && !target.countCards("he", (card) => {
  ...
  skill: {
    ...
    wuxinghelingshan_skill: {
      ...
      async content(event, trigger, player) {
        ...
        list.push("cancel2");
        const result = await player.chooseControl(list).set("prompt", get.prompt("wuxinghelingshan_skill")).set("prompt2", `将${get.translation(trigger.card)}转换为以下属性之一`).forResult();
    ...
    taigongyinfu_skill: {
      ...
      async content(event, trigger, player) {
        const result = await player.chooseCard("h", "是否发动【太公阴符】重铸一张手牌？", lib.filter.cardRecastable).set("ai", (card) => 5 - get.value(card)).forResult();
        if (result.bool) {
    ...
    taigongyinfu_link: {
      ...
      direct: true,
      async content(event, trigger, player) {
        const result = await player.chooseTarget("是否发动【太公阴符】横置或重置一名角色？").set("ai", (target) => get.effect(target, { name: "tiesuo" }, _status.event.player, _status.event.player)).forResult();
        if (result.bool) {
    ...
    yingbian_changeTarget: {
      ...
      async content(event, trigger, player) {
        if (trigger.yingbian_addTarget) {
          const result2 = await player.chooseTarget(`应变：是否为${get.translation(trigger.card)}增加一个目标？`, (card, player2, target) => {
            const evt = _status.event.getTrigger();
            const triggerCard = evt.card;
            return !evt.targets.includes(target) && lib.filter.targetEnabled2(triggerCard, player2, target) && lib.filter.targetInRange(triggerCard, player2, target);
          }).set("ai", (target) => {
        ...
        const result = await player.chooseTarget(`应变：是否为${get.translation(trigger.card)}减少一个目标？`, (card, player2, target) => {
          const evt = _status.event.getTrigger();
