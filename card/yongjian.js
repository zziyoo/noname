import { lib, _status, ui, get, game } from "noname";
const type = "card";
const yongjian = {
  name: "yongjian",
  connect: true,
  ...
  skill: {
    yitianjian: {
      ...
      async content(event, trigger, player) {
        const result = await player.chooseToDiscard("h", get.prompt("yitianjian"), "弃置一张手牌并回复1点体力").set("ai", (card) => 7 - get.value(card)).set("logSkill", "yitianjian").forResult();
        if (result.bool) {
    ...
    serafuku: {
      ...
      async content(event, trigger, player) {
        ...
        next.judge2 = (result2) => result2.bool === false;
    ...
    g_du: {
      ...
      async content(event, trigger, player) {
        ...
        if (typeof trigger.getl === "function") {
          num = trigger.getl(player).hs.filter((i) => get.name(i, player) === "du").length;
        }
    ...
    g_du_give: {
      ...
      async content(event, trigger, player) {
        const hs = player.getCards("h");
        const cards = trigger.name === "phase" ? player._start_cards.filter((card) => get.name(card, player) === "du" && hs.includes(card)) : trigger.cards.filter((card) => get.name(card, player) === "du" && hs.includes(card));
        ...
        while (cards.length) {
          const result = await player.chooseCardTarget({
            ...
            ai1(card) {
              ...
              if (["usedu", "keepdu"].some((tag) => player2.hasSkillTag(tag)) || get.effect(player2, { name: "losehp" }, player2, player2) > 0) {
                return 0;
              }
              if (!ui.selected.cards.length) {
                return 1;
            ...
            ai2(target2) {
              if (["usedu", "keepdu"].some((tag) => target2.hasSkillTag(tag))) {
                return get.attitude(_status.event.player, target2) - 0.01;
              }
              return -get.attitude(_status.event.player, target2) + 0.01;
            }
          }).forResult();
        ...
        for (const id in givenMap) {
          ...
          logs.push(source);
          map.push([source, givenMap[id]]);
          givenCards.addArray(givenMap[id]);
        }
        player.showCards(
          givenCards,
