  ...
  card: {
    ...
    lx_huoshaolianying: {
      ...
      async content(event, trigger, player) {
        ...
        if (target.countCards("he") === 1) {
          ...
          result = await player.choosePlayerCard(target, true, "he").set("ai", (button) => {
        ...
        await target.showCards(result.cards).setContent(() => {
        });
        ...
        await game.delay();
        const result2 = await player.chooseToDiscard({ suit: get.suit(result.cards[0]) }, (card) => {
        ...
        if (result2.bool) {
          const discards = result.cards.slice(0).filter((card) => lib.filter.canBeDiscarded(card, player, target, event));
          if (discards.length) {
          ...
          target(player, target) {
            ...
            if (_status.event.player === player) {
              if (target.isAllCardsKnown(player)) {
                if (!target.hasCards("h", (card) => {
                  return player.hasCards("h", (card2) => get.suit(card2) === get.suit(card));
                })) {
                  return 0;
                }
              }
    ...
    suibozhuliu: {
      ...
      async effect(event, trigger, player, result) {
        if (result.bool === false && player.hasCards("e")) {
          const result2 = await player.choosePlayerCard("e", player, true).set("filterButton", (button) => {
    ...
    yidugongdu: {
      ...
      async content(event, trigger, player) {
        ...
        if (player.hasCards("h", "du") && target.hasCards("h", "du")) {
          let chooseButton;
          if (player === target) {
          ...
          const result = await player.chooseButton(chooseButton, true).set("filterButton", (button) => {
            return get.name(button.link) === "du";
          }).set("ai", (button) => {
            const player2 = get.player();
            const target2 = get.owner(button.link);
            const eff = get.effect(target2, { name: "losehp" }, target2, player2);
            if (target2.hasSkillTag("usedu")) {
              return -2 * eff;
            }
        ...
        result: {
          target(player, target) {
            ...
            if (bool(player) && bool(target)) {
              const eff1 = (current) => current.hasSkillTag("usedu") ? 5 : current.hasSkillTag("nodu") ? 0 : get.effect(current, { name: "losehp" }, current, player);
              const eff = Math.max(eff1(player), eff1(target));
              return 4 + eff;
            }
    ...
    dajunyajing: {
      ...
      async content(event, trigger, player) {
        ...
        for (const current of targets) {
          ...
          if (!current.isIn() || !current.hasCards("hes") || current.hasSkill("diaohulishan")) {
            continue;
          }
    ...
    zj_lianjunshengyan: {
      ...
      async contentBefore(event, trigger, player) {
        ...
        if (!evt.lianjunType) {
          ...
          const result = await player.chooseControl("摸一张牌", "回复1点体力").set("prompt", `联军盛宴：选择一项令所有目标执行`).set("ai", () => get.event().val > 0 ? "摸一张牌" : "回复1点体力").set("val", val).forResult();
          if (result?.control) {
  ...
  skill: {
    tiejili_skill: {
      ...
      async content(event, trigger, player) {
        ...
        player.when("phaseAfter").step(async () => {
          const card = player.getEquip("tiejili");
          if (card) {
            player.storage.tiejili_skill = card.storage.tiejili_skill;
    ...
    ty_feilongduofeng_skill: {
      ...
      async cost(event, trigger, player) {
        ...
        choice.push("cancel2");
        const result = await player.chooseControl(choice).set("prompt", get.prompt(event.name.slice(0, -5), trigger.target)).set("choiceList", ["摸一张牌", "令其弃置一张牌"]).set(
          "res",
          (() => {
        ...
        ).set("ai", () => get.event().res).forResult();
        event.result = {
    ...
    shangfangbaojian_skill: {
      ...
      filter(event, player) {
        ...
        if (!event.player.hasCards("h") && !player.hasCards("h")) {
          return false;
        }
      ...
      async cost(event, trigger, player) {
        ...
        choice.push("cancel2");
        const result = await player.chooseControl(choice).set("prompt", get.prompt2(event.name.slice(0, -5), trigger.player)).set(
          "res",
          (() => {
            if (get.attitude(player, trigger.player) <= 0 && trigger.player.hasCards("h")) {
              return "获得其一张手牌";
            }
            if (!player.hasCards("h")) {
              return "cancel2";
      ...
      subSkill: {
        discard: {
          ...
          filter(event, player) {
            ...
            const cards = player.getEquips("shangfangbaojian");
    ...
    qingmingjian_skill: {
      ...
      async cost(event, trigger, player) {
        event.result = await player.chooseTarget(get.prompt2(event.name.slice(0, -5)), lib.filter.notMe).set("ai", (target) => {
    ...
    mengchong_skill: {
      ...
      async cost(event, trigger, player) {
        ...
        const num = player.storage.mengchong_skill;
        const result = await player.chooseControl("+1", "-1").set("prompt", `艨艟：请修改与其他角色计算距离（当前：${num > 0 ? "+" : ""}${num}）`).set("ai", () => {
          const player2 = get.player();
          const num2 = player2.countCards((card) => {
            return player2.hasUseTarget(card, true, true) && player2.hasUseValue(card, true, true);
          });
          return num2 > 2 ? "-1" : "+1";
        }).forResult();
        event.result = {
    ...
    liuyedao_skill: {
      ...
      async cost(event, trigger, player) {
        event.result = await player.chooseToDiscard(get.prompt2(event.skill), 2, "h").set("ai", (card) => {
          if (get.event().val <= 0) {
            return 0;
          }
          return 6 - get.value(card);
        }).set("val", get.damageEffect(trigger.player, trigger.source, player)).set("chooseonly", true).forResult();
      },
      async content(event, trigger, player) {
