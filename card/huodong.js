  ...
  card: {
    ...
    mb_qingnangshu: {
      ...
      async onEquip(event, trigger, player) {
        ...
        const cards = lib.card["mb_qingnangshu"].getCards(player);
        if (cards.length) {
      ...
      getCards(player) {
        ...
        const js = player.getVCards("j", (card) => card.storage?.equipEnable).flatMap((card) => card.cards.filter((card2) => card2.name === "mb_qingnangshu")).filter((card) => card.storage?.mb_qingnangshu_skill > 0 || typeof card.storage?.mb_qingnangshu_skill !== "number");
        return es.concat(js);
      },
      ai: {
        equipValue: 9
    ...
    jianhao: {
      ...
      async content(event, trigger, player) {
        ...
        do {
          ...
          const result = await target.chooseControl(choices).set("prompt", "见好就收：猜测下一张牌的点数大于或小于，或者取消获得所有展示过的牌").set("ai", () => {
            if (get.event().gain.length > 2) {
              return "cancel2";
            }
            return get.event().num < 7 ? 0 : 1;
          }).set("gain", gain).set("num", numx).forResult();
      ...
      async content(event, trigger, player) {
        ...
        if (cards.length) {
          ...
          for (const card of cards) {
            let tag = card.gaintag?.find((tag2) => tag2.startsWith(name));
            if (tag) {
    ...
    haoyun: {
      ...
      async content(event, trigger, player) {
        ...
        const { target } = event;
        while (true) {
          const result = await target.chooseControl("black", "red").set("prompt", `好运：选择一种颜色，然后开始判定。如果颜色为你选择的颜色，你获得此牌且重复此流程。`).set("ai", () => Math.random() > 0.4 ? "black" : "red").forResult();
          if (result?.control) {
            ...
            const judgeEvent = target.judge((card) => {
              ...
              if (event2.name !== "judge") {
                event2 = event2.getParent("judge", true);
              }
              if (get.color(card) === event2?.haoyun_color) {
                return 1.5;
              }
              return -1.5;
            });
            ...
            judgeEvent.set("callback", async (event2) => {
              if (event2.judgeResult.color === event2.getParent().haoyun_color) {
            ...
            humans.map((current) => {
              return new Promise((resolve, reject) => {
                if (current.isOnline()) {
                  ...
                  current.wait((result, player2) => {
                    solve(result, player2);
                    resolve(void 0);
                  });
                } else if (current === game.me) {
                  const next = lib.card.yifu.chooseBool(targets, current);
                  const solver = (result, player2) => {
                    solve(result, player2);
                    resolve(void 0);
                  };
    ...
    liehuo: {
      ...
      async content(event, trigger, player) {
        ...
        player.line(targets);
        const chooseEvent = player.chooseCardOL(targets, "烈火：请选择一张手牌", true).set("ai", (card2) => Math.random()).set("source", player);
        chooseEvent.aiCard = (target) => {
        ...
        const card = result[targets.indexOf(player)]?.cards[0];
        const damage = [];
        if (!card) {
          return;
        }
        for (const [i, target] of targets.entries()) {
          const color = get.color(result[i].cards[0], target);
          if (target !== player) {
    ...
    shenbing: {
      ...
      async contentBefore(event, trigger, player) {
        ...
        if (!evt.shenbing) {
          const result = await player.chooseControl("弃置牌", "使用牌").set("prompt", `神兵：令目标弃置装备区所有牌或依次使用牌堆不用副类型的装备牌各一张。`).set("ai", () => Math.random() > 0.5 ? "弃置牌" : "使用牌").forResult();
          if (result?.control) {
      ...
      async content(event, trigger, player) {
        ...
        if (evt.shenbing === "useCard") {
          for (let i = 1; i < 6; i++) {
            ...
            const card = get.cardPile2((card2) => get.subtype(card2) === `equip${i}` && target.canUse(card2, target));
            if (card) {
    ...
    fugui: {
      ...
      async content(event, trigger, player) {
        ...
        target.when(
          ...
          firstDo: true
        }).filter((evt) => evt.getg(target)?.length).step(async (event2, trigger2, player2) => {
          player2.removeSkill(event2.name);
          await source.draw(trigger2.getg(player2)?.length);
        }).finish();
      },
      ai: {
        wuxie() {
    ...
    tangying: {
      ...
      async content(event, trigger, player) {
        ...
        for (const phase of lib.phaseName) {
          ...
          if (evt2?.name === phase && !evt2.finished) {
            evt2.cancel(true, null, true);
            break;
          }
        }
        ...
        target.when({ player: "phaseBegin" }).step(async (event2, trigger2, player2) => {
          const result = await player2.chooseControl("摸牌阶段", "出牌阶段").set("prompt", "躺赢：选择要执行的额外阶段").set("ai", () => {
            const player3 = get.player();
            return player3.countCards("hs", (card) => player3.hasValueTarget(card)) < 2 ? "摸牌阶段" : "出牌阶段";
          }).forResult();
          if (result?.control) {
            const name = result.control === "摸牌阶段" ? "phaseDraw" : "phaseUse";
            trigger2.phaseList.splice(trigger2.num, 0, `${name}|${event2.name}`);
          }
        });
      },
      ai: {
    ...
    dashi: {
      ...
      filterTarget(card, player, target) {
        ...
        return ui.selected.targets.concat([target]).some((target2) => target2.countCards("h")) && player !== target;
      },
      selectTarget: 2,
      ...
      multicheck(card, player) {
        return game.hasPlayer((current) => {
          return current !== player && game.hasPlayer((currentx) => {
          ...
          target(player, target) {
            ...
            if (targets.length === 0) {
              for (const current of players) {
                if (current !== player && get.attitude(player, current) > 3) {
                  list.push(current);
                }
              }
              ...
              for (const current of players) {
                if (current !== player && get.attitude(player, current) < 1) {
                  list.push(current);
                }
              }
              ...
              for (const current of list) {
                if (from.countCards("h") >= current.countCards("h")) {
                  break;
                }
                const count = current.countCards("h") - from.countCards("h");
    ...
    khquanjiux: {
      ...
      async content(event, trigger, player) {
        ...
        for (const target2 of targets) {
          const num = get.rand(1, target2.countCards("h"));
          const cards = target2.getCards("h").randomGets(num);
          if (!cards.length) {
            continue;
          }
          target2.addGaintag(cards, "khquanjiux_tag");
          cards.forEach((card) => {
            game.broadcastAll((card2) => {
              if (!card2.storage?.khquanjiux) {
                card2.storage.khquanjiux = [card2.suit, card2.number, card2.name, card2.nature];
              }
              card2.init([card2.suit, card2.number, "jiu"]);
            }, card);
          });
        }
        let target;
        while (true) {
          ...
          const result = await target.chooseToRespond("劝酒：打出一张【酒】否则受到每名其他角色造成的1点伤害", (card) => get.name(card) === "jiu").set("ai", () => 114514).forResult();
          if (!result?.bool) {
    ...
    nisiwohuo: {
      ...
      async content(event, trigger, player) {
        ...
        await game.delayx();
        const targets = game.filterPlayer((target2) => target2 !== player).sortBySeat();
        if (!targets.length) {
          return;
        }
        player.line(targets);
        game.broadcastAll((event2) => {
          if (!Array.isArray(_status.nisiwohuo)) {
            _status.nisiwohuo = [];
          }
          _status.nisiwohuo.push(event2);
        }, event);
        let count = 0;
        const goon = () => {
        ...
        while (goon() && count < 100 && target?.isIn()) {
          ...
          ).set("ai2", (...args) => get.effect_use(...args) - get.event().effect).set("effect", get.effect(target, { name: "losehp" }, target, target)).set("addCount", false).forResult();
          if (!goon()) {
            break;
          }
          if (!result?.bool) {
    ...
    qixin: {
      ...
      async content(event, trigger, player) {
        ...
        const result = await target.chooseToMove(`兄弟同心：请分配${get.translation(player)}和你的手牌`, true, "allowChooseAll").set("list", [
          ...
          const cards12 = get.event().cards1;
          const cardx1 = cards12.filter((card) => card.name === "du");
          const cardy1 = cards12.removeArray(cardx1);
          const cards22 = get.event().cards2;
          const cardx2 = cards22.filter((card) => card.name === "du");
          const cardy2 = cards22.removeArray(cardx2);
          switch (get.sgn(att)) {
            case 1: {
              const cards = cards12.concat(cards22);
              const cardsx = cards.filter((card) => 8 - get.value(card, target2));
        ...
        }).set("cards1", cards1).set("cards2", cards2).forResult();
    ...
    chadao: {
      ...
      async content(event, trigger, player) {
        ...
        while (cards.length < 2) {
          const card = get.cardPile((card2) => get.is.damageCard(card2) && !cards.includes(card2));
          if (card) {
            cards.add(card);
          } else {
            break;
    ...
    khquanjiu: {
      ...
      async content(event, trigger, player) {
        const { target } = event;
        const result = await target.chooseToUse("劝酒：使用一张【酒】或点数为9的牌，否则失去1点体力", (...args) => {
          const card = args[0];
          if (get.name(card) !== "jiu" && get.number(card) !== "unsure" && get.number(card) !== 9) {
            return false;
          }
          return lib.filter.filterCard(...args);
        }).set("ai2", (...args) => get.effect_use(...args) - _status.event.effect).set("effect", get.effect(target, { name: "losehp" }, target, target)).set("addCount", false).forResult();
        if (!result.bool) {
          await target.loseHp();
        }
      },
    ...
    luojing: {
      ...
      async content(event, trigger, player) {
        ...
        player.when({ global: "dieAfter" }).filter((evtx) => evtx.player === target).step(async () => {
          await player.draw();
        });
      }
    },
    ...
    hongyun: {
      ...
      async content(event, trigger, player) {
        const target = event.target;
        const result = await target.chooseToDiscard(`红运当头：是否弃置至多两张牌然后获得等量红桃牌`, [1, 2], "he").set("ai", (card) => 6 - get.value(card)).forResult();
        if (result?.bool && result.cards) {
          const cards = [];
          while (cards.length < result.cards.length) {
            const card = get.cardPile((card2) => get.suit(card2) === "heart" && !cards.includes(card2));
            if (card) {
              cards.add(card);
            } else {
              break;
    ...
    leigong: {
      ...
      async content(event, trigger, player) {
        const target = event.target;
        target.when("damageBefore").filter((evt) => evt.getParent(event.name) === event && evt.card?.name === "shandian").step(async (evt, trigger2, player2) => {
          trigger2.card = event.card;
        });
  ...
  skill: {
    ...
    mb_qingnangshu_skill: {
      ...
      intro: {
        markcount(_, player) {
          const cards = lib.card["mb_qingnangshu"].getCards(player);
          const num = cards.reduce((sum, card) => {
      ...
      async content(event, trigger, player) {
        ...
        if (get.position(event.indexedData) === "e") {
          ...
          card = player.getCards("j", (card2) => {
            const vcard2 = card2[card2.cardSymbol];
            return vcard2?.storage?.equipEnable && vcard2.cards.some((c) => c === event.indexedData);
          })[0];
      ...
      async broadcast(card, vcard, player, inJudge) {
        ...
        if (!inJudge && get.is.ordinaryCard(vcard)) {
          game.broadcastAll(
            (vcard2, num) => {
              vcard2.cards[0].storage.mb_qingnangshu_skill = num;
            },
            vcard,
            vcard.storage.mb_qingnangshu_skill
          );
        ...
        if (vcard.storage.mb_qingnangshu_skill <= 0) {
          const cardx = inJudge ? vcard : card;
          if (player) {
            if (cardx) {
              if (inJudge) {
    ...
    jiaoyou_skill: {
      ...
      filter(event, player) {
        ...
        return player.hasHistory(
          "lose",
          (evtx) => evtx.getParent() === evt && Object.keys(evtx.gaintag_map).some((i) => {
      ...
      async content(event, trigger, player) {
        ...
        const evtx = player.getHistory(
          "lose",
          (evtx2) => evtx2.getParent() === evt && Object.keys(evtx2.gaintag_map).some((i) => {
      ...
      ai: {
        effect: {
          player_use(card, player, target) {
            ...
            const filter = (card2) => get.type(card2) === "equip" && get.info(card2)?.toself === false;
        ...
        if (targets.length === 1) {
          ...
          event.result = await player.chooseTarget(
            ...
            (card, player2, target) => {
              ...
              if (!player2.getStorage("youfu_skill").includes(target)) {
                return false;
              }
    ...
    luojing_skill: {
      ...
      async content(event, trigger, player) {
        player.chooseToUse(
          ...
          -1
        ).set("sourcex", trigger.player).set("filterTarget", (...args) => {
          const [card, player2, target] = args;
          if (target !== _status.event.sourcex) {
            return false;
          }
          return lib.filter.targetEnabled(...args);
    ...
    chadaox_skill: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = targets.length > 1 ? await player.chooseTarget(
          ...
          (card, player2, target) => {
            ...
            if (!target.isFriendOf(player2)) {
              target.prompt("不是队友", "fire");
              return false;
            }
            if ((trigger2._chadaox_skill_players || []).includes(target)) {
          ...
          true,
          1
        ).set("ai", (target) => get.damageEffect(target, get.event().getTrigger().source, target, get.event().getTrigger().nature)).forResult() : {
          bool: true,
          targets
        };
      },
    ...
    yifu_skill: {
      ...
      async content(event, trigger, player) {
        ...
        await target.chooseToGive(true, "he", player).set("prompt", `义父：选择一张牌孝敬给${get.translation(player)}`).forResult();
      },
      subSkill: {
        son: {
          intro: {
