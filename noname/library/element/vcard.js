...
class VCard {
  ...
  getCacheKey(similar) {
    ...
    if (!this.cards.length) {
      return prefix + `${this.name}+${this.suit ? this.suit : this.color || "none"}+${this.number === void 0 ? "none" : this.number}${this.nature ? "+" + this.nature : ""}]`;
    }
    if (similar !== void 0 && this.cards.length === 1) {
