            ...
            if (typeof card == "string") {
              ...
              if (info.ai && info.ai.order) {
                if (typeof info.ai.order == "number") {
                  ...
                  return info.ai.order(null, player2);
                }
              }
            }
            return 1;
...
card.${cardPackName}.${itemName}.${termName}`, term);
      ...
      default: {
        if (typeof animate === "function") {
          ...
          if (typeof animateResult === "number") {
            delay2 = game.delay(0, get.delayx(animateResult, animateResult));
          } else {
  ...
  addToExpansionMultiple: [
    ...
    async (event, trigger, player) => {
      ...
      for (const [id, cards] of Object.entries(event.gaining_map)) {
        ...
        const cards2 = cards.filter((card) => !hs.includes(card));
        ...
        switch (animate) {
          ...
          case "giveAuto": {
            ...
            for (const cardx of cards2) {
              ...
              if (owner) {
                const id2 = owner.playerid;
                if (!gainmap[id2]) {
                  gainmap[id2] = [];
                }
                gainmap[id2].push(cardx);
              }
            }
            if (animate === "give") {
              ...
              for (const id2 in gainmap) {
                const source = (_status.connectMode ? lib.playerOL : game.playerMap)[id2];
                if (evtmap[id2][1].length) {
                  source.$giveAuto(evtmap[id2][1], player2, false);
                  if (event.log) {
                    game.log(player2, "将", get.cnNumber(evtmap[id2][1].length), "张牌置于了武将牌上");
                  }
                }
                if (evtmap[id2][2].length) {
                  source.$give(evtmap[id2][2], player2, false);
                  if (event.log) {
                    game.log(player2, "将", evtmap[id2][2], "置于了武将牌上");
                  }
                }
              }
            }
  ...
  async showCards(event, trigger, player) {
    ...
    if (!flashAnimation) {
      ...
      const createDialog = (cards22, id, customButton2, event2) => {
        const dialog = get.idDialog(id);
        ...
        if (typeof customButton2 == "function") {
          dialog.buttons.forEach((button) => customButton2(button, event2));
        }
      };
      ...
      game.broadcast(
        (func, cards22, id, customButton2, event2) => {
          func(cards22, id, customButton2, event2);
        },
        createDialog,
        event.hiddencards,
  ...
  useCard: [
      ...
      event._playTargetAnimation = () => {
        ...
        if (event.card.name == "wuxie" && event.getParent()._info_map) {
          ...
          if (event.addedTarget) {
            player.line2(targets2.concat(event.addedTargets), config);
          } else if (get.info(event.card, false).multitarget && targets2.length > 1 && !get.info(event.card, false).multiline) {
            player.line2(targets2, config);
          } else {
            player.line(targets2, config);
          }
        }
      };
    ...
    async (event, trigger, player) => {
      ...
      event.sortTarget = (animate, sort) => {
        const { card: card2, targets: targets2, num: num2, fixedSeat } = event;
        const info = get.info(card2, false);
        if (num2 !== 0 || targets2.length <= 1) {
          return;
        }
        if (!info.multitarget && !fixedSeat && !sort) {
          targets2.sortBySeat(_status.currentPhase || player);
        }
        if (!animate) {
          return;
        }
      ...
      event.sortTarget();
      event._triggerTo = async (name, key, first, last) => {
        const { cards: cards2, targets: targets2 } = event;
        ...
        const triggeredTargets = event[key];
        ...
        event.redo();
        return;
        function getTriggerTarget(event2, triggered) {
          const sortedTargets = [...event2.targets];
          const remainingTargets = /* @__PURE__ */ new Map();
          for (const target2 of sortedTargets) {
            remainingTargets.set(target2, (remainingTargets.get(target2) || 0) + 1);
          }
          for (const target2 of triggered) {
            const count = remainingTargets.get(target2);
            if (count) {
              remainingTargets.set(target2, count - 1);
            }
          }
          sortedTargets.sortBySeat(_status.currentPhase || event2.player);
          for (const target2 of sortedTargets) {
            if ((remainingTargets.get(target2) || 0) > 0) {
              return target2;
            }
          }
          return null;
