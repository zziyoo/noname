    ...
    skill: {
      ...
      async content(event, trigger, player) {
        trigger.set("chooseToDiscard", async (event2, player2, target) => {
          const { discardPosition = "h", cards2, filterDiscard = { suit: get.suit(cards2[0]) } } = event2;
          const result = await player2.chooseCard(`请选择火攻要展示的牌`, discardPosition, filterDiscard).set("ai", () => Math.random()).forResult();
