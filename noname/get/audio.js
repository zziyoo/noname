...
class Audio {
  ...
  initAudioList() {
    ...
    if (result != void 0) {
      ...
      const result2 = this.parseAudio(this.name, this.#audioInfo);
      Audio.#audioCache[key] = result2;
      this.#audioList = JSON.parse(JSON.stringify(result2));
    }
  }
  ...
  parseAudio(name, audioInfo) {
    ...
    if (list) {
      let [, path2, audioNum, ext2] = list;
      if (path2 == void 0) {
        path2 = this.#Audio.defaultPath;
      } else {
        path2 = path2 + "/";
      }
      if (ext2 == void 0) {
      ...
      for (let i = 1; i <= parseInt(audioNum); i++) {
        audioList.push(this.#Audio.textMapWithIndex(path2, ext2, i));
      }
      return audioList;
    }
...
class SkillAudio {
  ...
  constructor(name, player, info, audioname) {
    ...
    if (this.info.logAudio2) {
      const key = this.getName((name2) => !!this.info.logAudio2?.[name2]);
    ...
    if (this.info.audioname2) {
      const key = this.getName((name2) => !!this.info.audioname2?.[name2]);
