  ...
  async waitPause() {
    if (_status.paused2 || _status.imchoosing) {
      if (!lib.status.dateDelaying) {
        lib.status.dateDelaying = /* @__PURE__ */ new Date();
      }
    }
    await Promise.all([this.pause, this.pause2, this.pause3, this.over, this.delay].filter((i) => i.isStarted));
    if (lib.status.dateDelaying) {
      lib.status.dateDelayed += lib.getUTC(/* @__PURE__ */ new Date()) - lib.getUTC(lib.status.dateDelaying);
...
class Deferred {
  ...
  resolve() {
    ...
    Promise.resolve().then(() => this.#resolver && this.#resolver()).then(() => {
      this.#promise = null;
      this.#resolver = null;
    });
  }
