...
${error}`);
  ...
  over(result, bool) {
    ...
    let i, j, k, num, table, tr, td, dialog, hsMap = /* @__PURE__ */ new Map([]), poptipData = /* @__PURE__ */ new Map([]);
    ...
    if (game.online) {
      ...
      const poptipData2 = arguments[2];
      if (poptipData2 instanceof Map) {
        const players = game.players.concat(game.dead, game.additionaldead || []);
        for (const target of players) {
          if (!poptipData2.has(target.playerid)) continue;
          const [id, hs] = poptipData2.get(target.playerid);
          lib.poptip.add({
            id,
            name: `<img style="width:15px; vertical-align: middle;" src="${lib.assetURL}image/card/handcard.png">`,
            dialog(dialog3) {
              dialog3.add(`${get.translation(target)}的手牌`);
              dialog3[hs.length ? "addSmall" : "addText"](hs.length ? hs : "（没有手牌）");
            }
          });
        }
      }
    ...
    for (let i2 = 0; i2 < clients.length; i2++) {
      if (clients[i2].isOnline2()) {
        clients[i2].send(game.over, dialog.content.innerHTML, game.checkOnlineResult(clients[i2]), poptipData);
      }
    }
    dialog.add(ui.create.div(".placeholder"));
  ...
  finishSkill(skillId, sub) {
    ...
    const deleteSkill = (skill, iInfo2) => {
