...
class Click {
  ...
  count(e) {
    ...
    const dialog = ui.create.dialog("hidden", `${get.translation(target)}的手牌`);
    ...
    const intervalId = setInterval(() => {
      const { cards: cards2, allShown: allShown2 } = getVisibleCards(target);
      if (!cards2.length && !allShown2) {
        closeCountDialog(target);
        return;
      }
      addCardsToCountDialog(container, cards2);
    }, 200);
    ...
    function getVisibleCards(target2) {
      const allShown2 = target2.isUnderControl(true, game.me) || !game.observe && game.me && game.me.hasSkillTag("viewHandcard", null, target2, true);
      if (allShown2) {
        return { cards: target2.getCards("h"), allShown: allShown2 };
      } else {
        const filterCard = (card) => {
          return get.is.shownCard(card);
        };
        return { cards: target2.getCards("h", (card) => filterCard(card)), allShown: allShown2 };
      }
    }
    function createHandCardsContainer(target2) {
      const container2 = document.createElement("div");
      container2.className = "count-handcards-container";
      container2.setAttribute("id", `count_handcards_container_${target2.playerid}`);
      const wheelCards = (e2) => {
        if (e2.deltaY !== 0) {
          e2.preventDefault();
          container2.scrollLeft += e2.deltaY;
        }
      };
      container2.addEventListener("wheel", wheelCards, { passive: false });
      const activateCards = (e2) => {
        const card = e2.target.closest(".count-handcards-container > *");
        if (!card) {
          return;
        }
      ...
      container2.addEventListener("touchstart", activateCards);
      container2.destroyContainer = () => {
        container2.removeEventListener("wheel", wheelCards);
        container2.removeEventListener("touchstart", activateCards);
        container2.delete();
      };
      return container2;
    }
    function closeCountDialog(target2) {
      if (_status.countDialogs[target2.playerid]) {
        const dialog2 = _status.countDialogs[target2.playerid];
        if (ui.window.querySelector(`#${dialog2.id}`)) {
          clearInterval(dialog2._intervalId);
          const container2 = dialog2.handcardsContainer;
          container2.destroyContainer();
          dialog2?.delete();
        }
        delete _status.countDialogs[target2.playerid];
        return true;
      }
      return false;
    }
    function addCardsToCountDialog(container2, cards2) {
      if (!container2.id.startsWith("count_handcards_container_")) {
        return;
      }
      if (container2.buttons?.length == cards2.length && (cards2.length == 0 || container2.buttons?.every((button, index) => button.link == cards2[index]))) {
        return;
      }
      if (typeof container2.replaceChildren == "function") {
      ...
      if (num) {
        ...
        const buttons = ui.create.buttons(cards2, "card", container2);
        container2.buttons = buttons;
