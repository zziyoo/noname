...
const skills = {
  ...
  stdgaobian: {
    ...
    async content(event, trigger, player) {
      ...
      let result = bool ? await target.chooseButton({
        createDialog: [
          "选择一项",
          [
            [
      ...
      if (result.links?.[0] == "sha") {
        const result2 = await target.chooseCardButton({
      ...
      backup: {
        ...
        precontent(event, trigger, player) {
          player.when("useCard").filter((evt) => evt.getParent() == event.getParent()).step(async (event2, trigger2, player2) => {
            ...
            if (trigger2.targets?.some((target) => target.countCards("h") === num)) {
  ...
  stdzengou: {
    ...
    async content(event, trigger, player) {
      ...
      if (cards2.length <= 2) {
        ...
        cards2.forEach((card) => card.addKnower(player));
      }
  ...
  stdyingmen: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseCard(
        ...
        true,
        2,
        (card) => ["basic", "trick"].includes(get.type(card))
      ).set("ai", (card) => get.player().getUseValue(card)).forResult();
  ...
  stdpingjian: {
    ...
    chooseButton: {
      dialog(event, player) {
        ...
        const list = get.inpileVCardList((info) => {
          return cards2.some((card) => get.name(card) == info[2] && get.nature(card) == info[3]);
        });
  ...
  stdtongli: {
    ...
    filter(event, player) {
      ...
      const num = game.filterPlayer().map((i) => i.getCards("ej").map((card) => get.suit(card))).flat().unique().length;
  ...
  stdjianjie: {
    ...
    async cost(event, trigger, player) {
      ...
      }).set("ai", (target) => get.attitude(get.player(), target) * target.countCards("he")).forResult();
    },
    async content(event, trigger, player) {
  ...
  stdchenghao: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player.chooseTarget(`将${get.translation(cards2)}交给一名角色`, true).set("ai", function(target) {
    ...
    async content(event, trigger, player) {
      ...
      if (game.hasPlayer((target) => target != player)) {
        ...
        if (result?.links?.length && result.targets?.length) {
          ...
          player.line(target);
          await target.gain(map[suit]).set("giver", player).set(
            "given",
  ...
  stdshejian: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseToDiscard(get.prompt2(event.skill, trigger.player), "h", 2, "chooseonly").set("ai", (card) => {
  ...
  stdgongqiao: {
    ...
    async content(event, trigger, player) {
      ...
      while (true) {
        ...
        cards2.push(card);
        await player.showCards(card, `${get.translation(player)}【工巧】亮出的第${get.cnNumber(cards2.length, true)}张牌`, true).set("clearArena", false);
  ...
  stdwanglu: {
    ...
    usable: 1,
    filterTarget(card, player, target) {
      return target.countDiscardableCards(player, "e", (card2) => get.type(card2) == "equip");
    },
    async content(event, trigger, player) {
      const { target } = event;
  ...
  stdjinhui: {
    ...
    async content(event, trigger, player) {
      ...
      if (game.hasPlayer((target) => target != player)) {
        const result = await player.chooseTarget(`锦绘：与一名其他角色依次使用其中一张牌（不能连续使用相同颜色的牌）`, true, lib.filter.notMe).set("ai", (target) => get.attitude(get.player(), target)).forResult();
        if (result?.targets?.length) {
          const [target] = result.targets;
          player.line(target);
          targets.add(target);
      ...
      const colors = [];
      for (const target of targets) {
        if (cards2.some((card) => target.hasUseTarget(card, true, false) && !colors.includes(get.color(card)))) {
          const result = await target.chooseCardButton("锦绘：请使用其中一张牌", cards2, true).set("filterButton", (button) => {
            return get.event().cards.includes(button.link);
          }).set(
            "cards",
            cards2.filter((card) => target.hasUseTarget(card, true, false) && !colors.includes(get.color(card)))
    ...
    async content(event, trigger, player) {
      ...
      if (game.hasPlayer((target) => target != player)) {
        const result = await player.chooseTarget(
          ...
          true,
          (card, player2, target) => {
            return player2 != target && target.countCards("h");
          }
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          if (game.hasPlayer((target2) => get.recoverEffect(target2, player, player) > 0)) {
            return 1;
          }
          return 0;
        }
  ...
  stdlianrong: {
    ...
    async cost(event, trigger, player) {
      const cards2 = game.filterPlayer2((target) => target != player).map((target) => trigger.getl?.(target)?.cards2?.filter((card) => get.suit(card) == "heart" && get.position(card, true) == "d")).flat();
    ...
    chooseButton: {
      ...
      backup(links, player) {
        return {
          popname: true,
          filterCard(card, player2) {
            return get.type(card, player2) === "equip";
          },
          selectCard: 1,
          check(card) {
            return 7 - get.value(card);
  ...
  stdlijun: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), [1, player.getHp()], (card, player2, target) => {
        return target.countCards("h");
      }).set("ai", (target) => {
  ...
  stdwuwei: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseCardTarget({
        ...
        filterTarget(card, player2, target) {
          if (get.position(card) == "e" && target == player2) {
            return false;
          }
        ...
        ai2(target) {
          const card = ui.selected.cards[0], att = get.attitude(get.player(), target), range = target.getAttackRange(), cardrange = 1 - (get.info(card, false)?.distance?.attackFrom || 0);
    ...
    async content(event, trigger, player) {
      const card = event.cards[0], target = event.targets[0], range = target.getAttackRange();
      await target.equip(card);
  ...
  stdleiruo: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        return target != player2 && target.countGainableCards(player2, "e");
      }).set("ai", (target) => {
    ...
    async content(event, trigger, player) {
      ...
      if (target.canUse(card, player, false, false)) {
        const result = await target.chooseBool(`羸弱：是否视为对${get.translation(player)}使用一张无距离限制的【杀】`).set("choice", get.effect(player, card, target, target) > 0).forResult();
        if (result?.bool) {
          await target.useCard(card, player, false);
        }
      }
  ...
  stddechong: {
    ...
    async cost(event, trigger, player) {
      const result = await player.chooseCard(
        ...
        (card, player2) => {
          ...
          return lib.filter.canBeGained(card, target, player2);
        },
        "allowChooseAll"
      ).set("ai", (card) => {
        const player2 = get.player(), target = get.event().getTrigger().player, att = get.attitude(player2, target);
        if (att <= 0) {
          const num = target.countCards("h", (card2) => !target.hasValueTarget(card2));
          if (num >= target.hp) {
            return 6 - get.value(card);
          }
          return 0;
    ...
    async content(event, trigger, player) {
      const cards2 = event.cost_data, target = event.targets[0];
      await player.give(cards2, target);
      player.when({ global: "phaseDiscardBegin" }).filter((evt) => evt.player == target).step(async (event2, trigger2, player2) => {
        ...
        if (target2.countCards("h") >= target2.hp) {
          const result = await player2.chooseBool(`得宠：是否对${get.translation(target2)}造成1点伤害`).set("choice", get.damageEffect(target2, player2, player2) > 0).forResult();
          if (result?.bool) {
  ...
  xuanhuafu_skill: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer(
        (target) => get.distance(target, event.target) == 1 && lib.filter.targetInRange(card, player, target) && lib.filter.targetEnabled2(card, player, target) && !event.targets.includes(target)
      );
    },
    async cost(event, trigger, player) {
      const card = trigger.card;
      const targets = game.filterPlayer(
        (target) => get.distance(target, trigger.target) == 1 && lib.filter.targetInRange(card, player, target) && lib.filter.targetEnabled2(card, player, target) && !trigger.targets.includes(target)
      );
      if (targets.length == 1) {
        event.result = { bool: true, targets };
      } else {
        event.result = await player.chooseTarget(`宣花斧：为${get.translation(card)}额外指定一个目标`, true, (card2, player2, target) => {
          return get.event().targetsx.includes(target);
        }).set("targetsx", targets).set("ai", (target) => get.effect(target, get.event().getTrigger().card, get.player(), get.player())).forResult();
      }
    },
    async content(event, trigger, player) {
      const { targets } = event;
  ...
  stdyimou: {
    ...
    async content(event, trigger, player) {
      const target = event.targets[0], cards2 = event.cost_data;
      await player.give(cards2, target);
    }
  },
  stdmutao: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        return target.countCards("h");
      }).set("ai", (target) => get.damageEffect(target, get.player(), get.player())).forResult();
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
      await target.showHandcards();
      ...
      backup(links, player) {
        return {
          ...
          filterCard(card, player2) {
            ...
            return get.number(card, player2) + selected.reduce((sum, card2) => sum + get.number(card2, get.player()), 0) <= 13;
          },
          ...
          ai1(card) {
            ...
            if (sum + get.number(card, player2) == 13) {
              return 7 - get.value(card);
            }
            return 6 - get.value(card);
          },
  ...
  stdyizheng: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        return target.hp >= player2.hp && player2.canCompare(target);
      }).set("ai", (target) => {
    ...
    async content(event, trigger, player) {
      ...
      if (result.winner) {
        const winner = result.winner, loser = winner == player ? target : player;
        winner.line(loser);
  ...
  stdtaoluan: {
    ...
    async cost(event, trigger, player) {
      const result = await player.chooseCard(get.prompt2(event.skill, trigger.player), "he").set("ai", (card) => {
        const target = get.event().getTrigger().player, att = get.attitude(get.player(), target), shan = target.countCards("h", "shan");
    ...
    async content(event, trigger, player) {
      const cards2 = event.cost_data, target = trigger.player;
      await player.give(cards2, target);
      await target.showHandcards();
  ...
  stdcansi: {
    ...
    trigger: { player: "phaseZhunbeiBegin" },
    filter(event, player) {
      return player.countCards("he") && game.hasPlayer((target) => player.inRange(target));
    },
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(`残肆：令攻击范围内的一名角色获得你一张牌，然后视为对其依次使用【杀】,【决斗】`, true, (card, player2, target) => {
        return player2.inRange(target);
      }).set("ai", (target) => {
        const cards2 = [get.autoViewAs({ name: "sha", isCard: true }), get.autoViewAs({ name: "juedou", isCard: true })];
        return cards2.reduce((eff, card) => eff + get.effect(target, card, get.player(), get.player()), 0);
      }).forResult();
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
  ...
  stdmingshi: {
    ...
    chooseButton: {
      ...
      check(button) {
        ...
        if (link == "damage") {
          return game.filterPlayer().map((target) => get.damageEffect(target, player, player)).sort((a, b) => b - a)[0];
        }
        if (button.link == "move") {
          return 2;
        }
          ...
          async content(event, trigger, player2) {
            ...
            switch (link) {
              ...
              case "recover":
                ...
                break;
              case "damage": {
                const result = await player2.chooseTarget(`明识：对一名角色造成1点伤害`, true).set("ai", (target) => {
  ...
  stdmizhao: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), 2, lib.filter.notMe).set("targetprompt", ["传诏对象", "讨伐对象"]).set("ai", (target) => {
        const player2 = get.player();
        ...
        if (ui.selected.targets.length) {
          ...
          if (targets.length) {
            return get.effect(target, { name: "losehp" }, player2, target1, player2);
          }
        }
        return att;
  ...
  stdwufei: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        ...
        const player2 = get.player();
        const att = Math.sign(get.attitude(player2, target));
    ...
    async content(event, trigger, player) {
      ...
      await target.showHandcards();
      const list = [], bannedList = [], indexs = Object.keys(lib.color), hs = target.getCards("h");
      ...
      for (let i = 0; i < list.length; i++) {
        const colorx = list[i];
        const cards2 = hs.filter((card) => get.color(card, target) == colorx);
        if (cards2.length) {
          dialog.addArray([`<span class="text center">${get.translation(colorx)}</span>`, cards2]);
        }
      }
      const result = list.length > 1 ? await target.chooseControl(list).set("ai", () => {
        const { player: player2, controls } = get.event();
        const cards2 = player2.getCards("h");
        return controls.sort((a, b) => {
          return get.value(cards2.filter((card) => get.color(card) === a)) - get.value(cards2.filter((card) => get.color(card) === b));
        })[0];
      ...
      if (control) {
        ...
        const cards2 = target.getCards("h").filter((card) => get.color(card) === control);
        if (cards2.length) {
  ...
  stdzongxuan: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        return target.countDiscardableCards(player2, "ej");
      }).set("ai", (target) => {
        const player2 = get.player();
  ...
  stdzhiyan: {
    audio: "zhiyan",
    getcards: () => get.discarded().filterInD("d").filter((card) => get.type(card) == "equip"),
    trigger: { player: "phaseJieshuBegin" },
    filter(event, player) {
  ...
  stdbingzheng: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        ...
        return get.effect(target, { name: "guohe_copy2" }, player2, player2) + get.effect(player2, { name: "losehp" }, player2, player2);
      }).forResult();
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
  ...
  stdzhennan: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseToDiscard(get.prompt2(event.skill, trigger.player)).set("ai", (card) => {
  ...
  stdshuyong: {
    ...
    frequent: true,
    filter(event, player) {
      const target = event.player, last = target.getLastUsed(1);
  ...
  stdlirang: {
    getCards(event) {
      ...
      }).reduce((cards2, evt) => cards2.addArray(evt.cards.filterInD("d")), []).filter((card) => get.color(card) == "red");
    },
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseCard(get.prompt(event.skill, target), "he", str).set("ai", (card) => {
        ...
        return 0;
      }).set("targetx", target).set("cardsx", cards2).forResult();
    },
    async content(event, trigger, player) {
  ...
  stdqingcheng: {
    ...
    filterOk() {
      const {
        cards: cards2,
        targets: [target]
      } = ui.selected, player = get.player(), canAdd = (current, card) => {
        const lebu = get.autoViewAs({ name: "lebu", cards: [card] }, [card]);
        return lib.filter.judge(lebu, player, current);
      };
      return canAdd(player, cards2[0]) && canAdd(target, cards2[1]);
    },
    check(card) {
      return 8 - get.value(card);
    },
    ...
    async content(event, trigger, player) {
      ...
      for (let i = 0; i < cards2.length; i++) {
        const card = get.autoViewAs({ name: "lebu", cards: [cards2[i]] }), target = targets[i];
        if (player == target && player.canAddJudge(card) || player != target && player.canUse(card, target)) {
          await player.useCard(card, [cards2[i]], targets[i]);
        }
      }
    },
    ai: {
  ...
  stdmumu: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseToDiscard(get.prompt2(event.skill), "h").set("ai", (card) => {
        if (get.event().goon) {
          return 6 - get.value(card);
        }
        return 0;
  ...
  stdqijian: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), 2).set("filterTarget", (card, player2, target) => {
        if (!ui.selected.targets.length) {
          return true;
        }
        const targetx = ui.selected.targets[0];
        return targetx.countCards("h") + target.countCards("h") == 7;
      }).set("complexTarget", true).set("ai", (target) => {
        const player2 = get.player();
        if (!ui.selected.targets.length) {
    ...
    async content(event, trigger, player) {
      ...
      for (const target of targets) {
        ...
        if (goon) {
          result = await target.chooseControl(list).set("prompt", `七笺：弃置${get.translation(targetx)}一张牌或令其摸一张牌`).set("ai", () => {
  ...
  stdzhanjue: {
    ...
    ai: {
      ...
      skillTagFilter(player, tag, arg) {
        if (tag === "nokeep") {
          return (!arg || arg.card && get.name(arg.card) === "tao") && player.isPhaseUsing() && !player.countSkill("stdzhanjue") && player.hasCard((card) => {
  ...
  stdbaobian: {
    ...
    async content(event, trigger, player) {
      await player.loseHp();
      const target = event.targets[0], card = { name: "sha", isCard: true };
      const { cards: cards2 } = await target.chooseToDiscard("h", true).forResult();
      if (get.type(cards2[0]) == "basic" && player.canUse(card, target, false)) {
        await player.useCard(card, target, false);
      }
    }
  },
  ...
  stdmingjian: {
    ...
    delay: false,
    async content(event, trigger, player) {
      const cards2 = event.cards, target = event.target;
  ...
  stdpolu: {
    ...
    async cost(event, trigger, player) {
      const { bool, links: cards2 } = await player.choosePlayerCard("e", trigger.player, get.prompt2(event.skill, trigger.player)).set("ai", (button) => {
        const target = get.event().getTrigger().player, player2 = get.player();
  ...
  stdchoulve: {
    ...
    async content(event, trigger, player) {
      const cards2 = event.cards, target = event.target;
      await player.give(cards2, target);
      const { bool, cards: cardx } = await target.chooseCard(`是否展示并交给${get.translation(player)}一张装备牌？`, "he").set("filterCard", (card) => get.type(card) == "equip").set("ai", (card) => {
        if (get.event().att <= 0) {
          return 0;
        }
        return 5 - get.value(card);
  ...
  stdjiaozhao: {
    ...
    async content(event, trigger, player) {
      ...
      if (event.getParent(2).name == "stddanxin") {
        ...
        if (result2?.bool && result2?.links?.length) {
          await player.gain(result2.links, target, "giveAuto");
        }
      } else {
        if (!player.countCards("h")) {
          return;
        ...
        const result2 = await player.chooseButton(2, ["你的手牌", player.getCards("h"), `${get.translation(target)}展示的手牌`, cards2]).set("filterButton", (button) => {
        ...
        if (result2?.bool && result2?.links?.length) {
          const cards1 = result2.links.filter((card) => !cards2.includes(card)), cards22 = result2.links.filter((card) => cards2.includes(card));
          await player.swapHandcards(target, cards1, cards22);
        }
      }
    },
    ai: {
  ...
  stddanxin: {
    ...
    trigger: { player: "damageEnd" },
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill)).set("filterTarget", (card, player2, target) => {
        return target != player2 && target.countCards("h") > 1;
      }).set("ai", (target) => {
        return get.effect(target, { name: "shunshou_copy2" }, get.player(), get.player());
      }).forResult();
    },
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        ...
        for (let current of trigger2.targets) {
          eff += get.effect(current, trigger2.card, target, player2);
        }
  ...
  stdyirang: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        ...
        return !game.hasPlayer((current) => {
          return current != player2 && current.countCards("h") < target.countCards("h");
        });
    ...
    async content(event, trigger, player) {
      const cards2 = player.getCards("h"), target = event.targets[0];
      let types = [];
      for (let i = 0; i < cards2.length; i++) {
  ...
  stdshuangdao: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseTarget(get.prompt2(event.skill), function(card, player2, target) {
        ...
        if (_status.event.goon && get.attitude(player2, target) < 0) {
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        if (game.hasPlayer(function(current) {
          ...
          const result2 = await player.chooseTarget("是否对至多两名与其距离为1的角色各使用一张杀？", [1, 2], function(card, player2, target2) {
            if (!player2.canUse("sha", target2, false)) {
              return false;
            }
            return get.distance(get.event().identity, target2) <= 1;
          }).set("ai", function(target2) {
            let player2 = _status.event.player;
            return get.effect(target2, { name: "sha" }, player2, player2);
          }).set("identity", target).forResult();
          if (result2.bool) {
  ...
  stdjuece: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt(event.skill), "对一名本回合失去过至少两张牌的角色造成1点伤害", function(card, player2, target) {
  ...
  stdyuanqing: {
    ...
    async content(event, trigger, player) {
      for (const target of event.targets) {
        let cards2 = lib.skill.stdyuanqing.getCards(target);
        if (!cards2.length) {
          continue;
        }
        const result = await target.chooseButton(["获得其中一张牌", cards2], true).forResult();
        if (result.bool) {
  ...
  stdxingfa: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt(event.skill), "对一名其他角色造成1点伤害", function(card, player2, target) {
  ...
  stdyinge: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target.chooseTarget("选择使用杀的目标", true).set("useTargets", targets).set("filterTarget", (card, player2, target2) => {
        let targets2 = get.event().useTargets;
        return targets2.includes(target2);
      }).set("ai", (target2) => {
        return get.effect(target2, { name: "sha", isCard: true }, get.player(), get.player());
      }).forResult();
      if (result.bool) {
  ...
  stdjujian: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), lib.filter.notMe).set("ai", (target) => {
        const player2 = get.player();
        return target.getUseValue(_status.event.getTrigger().cards[0]) * get.attitude(player2, target);
      }).forResult();
    ...
    async content(event, trigger, player) {
      ...
      if (discards.length) {
        ...
        if (game.hasPlayer((current) => current != player && current.countCards("he"))) {
          const result2 = await player.chooseTarget(`获得至多${discards.length}名其他角色各一张牌`, [1, discards.length], true, function(card, player2, target) {
  ...
  stdfengbai: {
    ...
    getIndex(event, player) {
      ...
      return game.filterPlayer((current) => {
        ...
        game.log(evt.es);
        return evt.es.some((card) => cards2.includes(card));
      }).sortBySeat();
    },
    async content(event, trigger, player) {
      await event.targets[0].draw();
  ...
  stdjujin: {
    ...
    zhuSkill: true,
    async cost(event, trigger, player) {
      event.result = await player.chooseToDiscard(get.prompt2(event.skill), 2, "he").set("ai", (card) => {
        const player2 = get.player();
  ...
  stdqiuyuan: {
    ...
    async content(event, trigger, player) {
      ...
      const { card } = trigger;
      const result = await target.chooseToGive(`交给${get.translation(player)}一张牌，或成为${get.translation(card)}的额外目标`, player).set("ai", (card2) => {
        const { player: player2, target: target2 } = get.event();
  ...
  stdzhuikong: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseCard(get.prompt(event.skill, trigger.player), "使用一张【杀】与其拼点", (card) => {
        return get.name(card) == "sha";
      }).set("ai", (card) => {
  ...
  stdxiongyi: {
    ...
    async content(event, trigger, player) {
      ...
      while (true) {
        let stop = false;
        for (const target of targets) {
          let next = target.chooseToUse(function(card) {
            const event2 = get.event();
            if (!lib.filter.cardEnabled(card, event2.player, event2)) {
              return false;
            }
            return get.name(card) == "sha";
          }, "雄异：是否使用一张不可被响应的【杀】？").set("oncard", (card) => {
  ...
  stdfengpo: {
    ...
    trigger: { source: "damageBegin1" },
    filter(event, player) {
      return event.card?.name == "sha" && [player, event.player].some((target) => {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        ...
        if (aim === player2 && player2.getDiscardableCards(player2, "he", (card) => get.suit(card) == "diamond")) {
          eff /= 4;
        }
        return eff + get.effect(target, { name: "guohe" }, player2, player2);
      }).forResult();
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
      const result = await player.discardPlayerCard(target, "he", true).set("ai", (button) => {
        const suit = get.suit(button.link);
        return get.event().att * (suit == "diamond" ? 5 : 1) * get.value(button.link, player);
      }).set("prompt", "凤魄：弃置" + (target != player ? get.translation(target) : "") + "一张牌").set("prompt2", "若弃置了方片牌，则此伤害+1").set("att", get.sgnAttitude(player, target)).forResult();
      if (result.bool) {
  ...
  stddaoshu: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        const event2 = get.event().getTrigger();
        return target != event2.player && target.countCards("h");
      }).set("ai", (target) => {
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        if (cards2.length) {
          ...
          await trigger.player.gain(cards2, target, "give");
          const suits = cards2.reduce((list, card) => {
  ...
  stdxiongxia: {
    ...
    subSkill: {
      effect: {
        ...
        trigger: { player: "useCardAfter" },
        filter(event, player) {
          return event.skill == "stdxiongxia" && (event.targets || []).every((target) => {
  ...
  stdhuiji: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(
        ...
        (card, player2, target) => {
          const event2 = get.event().getTrigger();
          return !event2.targets.includes(target) && lib.filter.targetEnabled2(event2.card, player2, target) && lib.filter.targetInRange(event2.card, player2, target);
        },
        [1, 2]
      ).set("ai", (target) => {
        const player2 = get.event().player, event2 = get.event().getTrigger();
        return get.effect(target, event2.card, player2);
      }).forResult();
    },
    async content(event, trigger, player) {
  ...
  stdchunlao: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseTarget(get.prompt(event.skill), "用" + get.translation(cards2) + "交换一名其他角色的手牌", (card, player2, target) => target != player2).set("ai", (target) => {
        const { cards: cards3, player: player2 } = get.event();
        const att = get.attitude(player2, target);
        return (cards3.length - target.countCards("h") + 0.1) * att;
      }).set("cards", cards2).forResult();
    },
    async content(event, trigger, player) {
      const cards2 = trigger.cards, target = event.targets[0];
      await target.loseToDiscardpile(target.getCards("h"));
      await target.gain(cards2, "gain2").set("giver", player);
      if (player.isDamaged()) {
        const { bool } = await target.chooseBool("是否令" + get.translation(player) + "回复1点体力？").set("choice", get.recoverEffect(player, target, target) > 0).forResult();
        if (bool) {
          target.line(player);
  ...
  stdshefu: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseCard(get.prompt(event.skill), "将一张手牌置于武将牌上", "h").set("ai", (card) => {
        return (lib.card.list.slice().map((list) => list[2]).filter((name) => {
          return card.name == name;
        }).length - 1) / (get.value(card) || 0.5);
      }).forResult();
    },
    async content(event, trigger, player) {
  ...
  stdbazhan: {
    ...
    async content(event, trigger, player) {
      ...
      if (target.countCards("h")) {
        await target.chooseToGive(player, (card, player2) => {
      ...
      result: {
        target(player, target) {
          ...
          if (target.countMark("stdzhanying_count") == target.countCards("h") + 1) {
            const cards2 = player.getCards("hs", (card) => {
            ...
            return -cards2.reduce((sum, card) => sum + get.effect(target, card, player, player), 0);
          }
          return 1;
        }
      }
