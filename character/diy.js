...
const cards = {
  nsfuzhou_card: {
    ...
    async effect({ cards: cards2, _result: result2, player }) {
      const source = cards2[0].storage.nsfuzhou_source;
...
const skills = {
    ...
    async content(event, trigger, player) {
      ...
      await game.doAsyncInOrder(targets, async (target) => {
        ...
        if (!cards3?.length) {
          ...
          result2 = await target.chooseToGive(player, "he", `共创：是否交给${get.translation(player)}一张牌否则令其获得2枚“漏洞”（至多24枚）`).set("ai", (card) => {
            const { sourcex, player: player2 } = get.event();
            const att = get.attitude(player2, sourcex);
            if (sourcex == player2) {
            ...
            return 5 - get.value(card, player2);
      ...
      while (assigned.length < 4 && player.hasCard((card) => !card.hasGaintag("noname_gongchuang_assigned"), "he")) {
        ...
        if (result2?.bool && result2.cards?.length && result2.targets?.length) {
          ...
          player.addGaintag(cards3, "noname_gongchuang_assigned");
  ...
  huan_duocai: {
    ...
    async content(event, trigger, player) {
      ...
      const { target } = event;
      const skills2 = get.info(event.name).getSkills(player, target);
      let result2;
      if (skills2.length == 1) {
        result2 = { bool: true, links: skills2 };
      } else {
        result2 = await player.chooseButton([`多彩：获得${get.translation(target)}的一个技能`, [skills2, "skill"]], true).set("ai", (button) => get.skillRank(button.link, "inout")).forResult();
      }
      const { links } = result2;
      if (links?.length) {
        const [skill] = links;
  ...
  nspianwu: {
    skillTrigger(triggerName, player, skill) {
      ...
      next.player = player;
      next.skill = skill;
      next.setContent(async (event, trigger, player2) => {
        ...
        async content(event, trigger, player) {
          switch (event.triggername) {
            ...
            case "shijian_addSkill": {
              ...
              newSkill.filter = function(event2, player2, name) {
                if (!event2.player.isAlive()) {
                  return false;
                }
                if (typeof randomTrigger.filter == "function") {
                  if (randomTrigger.filter(event2, player2, name) == false) {
                    return false;
                  }
                }
                if (typeof randomSkillContent.filter == "function") {
                  if (randomSkillContent.filter(event2, player2, name) == false) {
                    return false;
                  }
                }
                if (randomSkillFilter) {
                  return randomSkillFilter.filter(event2, player2, name);
                }
                return true;
              };
              ...
              newSkill.check = function(event2, player2) {
                let result3 = 0;
                ...
                if (randomTrigger.num && lib.skill.nspianwu_global.skillContentList_hasNum.includes(randomSkillContent)) {
                  ...
                  if (contentResult.player) {
                    if (typeof contentResult.player == "function") {
                      result3 += contentResult.player(player2, event2.name);
                    } else {
              ...
              next.set("ai", (target) => {
                ...
                if (randomInit) {
                  if (typeof randomInit.result.player == "function") {
                    ...
                    initResult += randomInit.result.player(player2);
                ...
                if (rs.player) {
                  if (typeof rs.player == "function") {
                    ...
                    contentResultOfMe += rs.player(player2);
                  } else {
                  ...
                  if (typeof rs.evtPlayer == "function") {
                    ...
                    if (toTarget) {
                      contentResultOfMe += rs.evtPlayer(player2);
                    }
                  } else {
                    if (toTarget) {
                ...
                const mySkillLength = player2.skills.filter((skill) => skill.indexOf("ns_shijian_createSkill_") == 0).length;
                ...
                if (contentResultOfMe > 0 && initResult > -1) {
                  ...
                  if (game.countPlayer((current) => get.attitude(player2, current) > 3) > 0) {
                    return att;
                  }
                  return 0;
                }
  ...
  nshanlang: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseTarget([1, 3], get.prompt(event.skill), "和至多三名角色进行拼点", function(card, player2, target) {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseBool("是否令" + get.translation(target) + "获得一张牌？").set("goon", get.attitude(player, target) > 0).set("ai", () => _status.event.goon).forResult();
      if (result2.bool) {
  ...
  nskuanhuai: {
    ...
    subSkill: {
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          let result2;
          const cards2 = player.getHistory("lose", (evt) => evt.type === "discard" && evt.getParent("phaseDiscard") === trigger).flatMap((evt) => evt.cards2).filter((card) => get.type(card, null, false) === "basic" && get.position(card, true) === "d");
          while (true) {
            const cards22 = cards2.filter((card2) => get.position(card2, true) == "d" && player.hasUseTarget(card2));
            if (!cards22.length) {
              return;
            }
  ...
  nsdingbian: {
    ...
    async content(event, trigger, player) {
      ...
      if (goon) {
        result2 = await player.chooseControl().set("choiceList", ["从牌堆中获得一张基本牌", "令一种基本牌于本回合内不计入手牌上限"]).set("prompt", "定边：请选择一项").set("ai", () => {
          const player2 = _status.event.player;
          const list2 = ["tao", "shan"];
          const list22 = player2.getStorage("nsdingbian_ignore");
          list2.removeArray(list22);
          if (!list2.length) {
            return 0;
          }
          const num1 = player2.countCards("hs", (card) => {
            return get.type(card) != "basic" && player2.hasValueTarget(card, null, true);
          });
      ...
      result2 = await player.chooseButton(["令一种基本牌于本回合内不计入手牌上限", [list, "vcard"]], true).set("ai", (button) => {
        const name = button.link[2];
        const player2 = _status.event.player;
        if (name == "sha") {
          return 0;
        }
        const cards2 = player2.getCards("h", name);
        if (!cards2.length) {
          return 0;
        }
      ...
      if (result2.bool && result2.links?.length) {
        player.markAuto("nsdingbian_ignore", [result2.links[0][2]]);
      }
    },
    subSkill: {
      mark: {
  ...
  nstuilun: {
    ...
    async content(event, trigger, player) {
      ...
      if (player.hp == 2) {
        ...
        result2 = await player.chooseNumbers({
          list: [
            {
              min: 1,
              max: player.hp - 1,
      ...
      await player.loseHp(result2.numbers[0]);
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          if (player.hp < trigger.player.hp) {
            result2 = await player.chooseTarget("退论：是否令一名角色回复或失去1点体力？").set("ai", (target) => {
            ...
            if (result2.bool) {
              ...
              if (target.isHealthy()) {
                ...
                result2 = await player.chooseControl("回复1点体力", "失去1点体力").set("prompt", "令" + get.translation(target) + "…").set("ai", () => {
          ...
          if (trigger.player.countCards("h") > player.countCards("h")) {
            result2 = await player.chooseTarget("退论：是否令一名角色摸一张牌或弃置一张牌？").set("ai", (target) => {
            ...
            if (result2.bool) {
              ...
              if (!target.countCards("he")) {
                ...
                result2 = await player.chooseControl("摸一张牌", "弃置一张牌").set("prompt", "令" + get.translation(target) + "…").set("ai", () => get.attitude(player, target) > 0 ? 0 : 1).forResult();
              }
              if (result2.index == 0) {
                await target.draw();
              } else {
  ...
  nsshizui: {
    ...
    async cost(event, trigger, player) {
      ...
      next.set("val1", cards2.length ? get.value(cards2, player) : 0);
    ...
    async content(event, trigger, player) {
      ...
      const suit1 = get.suit(cards2[0], player);
  ...
  nsxiaoye: {
    ...
    async cost(event, trigger, player) {
      ...
      const result2 = await player.chooseButton([get.prompt(event.skill), [list, "vcard"]]).set("filterButton", function(button) {
  ...
  nsshimeng: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await target.chooseToUse("使用一张【杀】，或摸一张牌", function(card, player2) {
  ...
  noname_duocai: {
    ...
    async content(event, trigger, player) {
      ...
      const cards2 = game.filterPlayer2((i) => i !== player && trigger.getd(i, "cards2").length).flatMap((target) => trigger.getd(target, "cards2"));
      result2 = await player.chooseButton({
      ...
      const filterTarget = (card, player2, target) => {
        return target !== player2 && target.countDiscardableCards(player2, "hej") > 0;
      };
      ...
      result2 = await player.chooseTarget({
        ...
        ai(target) {
          const player2 = _status.event.player;
          return get.effect(target, { name: "guohe" }, player2, player2);
        }
      }).forResult();
  ...
  nsqingde: {
    ...
    async content(event, trigger, player) {
      ...
      if (event.triggername == "damageSource") {
        result2 = await player.chooseBool("是否令" + get.translation(target2) + "摸两张牌？").set("ai", () => get.attitude(player, target2) > 0).forResult();
        if (result2.bool) {
      ...
      result2 = await player.chooseBool("是否令" + get.translation(target2) + "回复1点体力？").set("ai", () => get.attitude(player, target2) > 0).forResult();
      if (result2.bool) {
  ...
  nsyidi: {
    ...
    async content(event, trigger, player) {
      ...
      await player.give(cards2, target, "visible").forResult();
      if (get.type(cards2[0], player) != "basic") {
  ...
  nsfuzhou: {
    ...
    async content(event, trigger, player) {
      ...
      cards2[0].storage.nsfuzhou_source = player;
      await game.delayx();
    },
    ai: {
      order: 5,
  ...
  nsguidao: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseCard(`${get.translation(trigger.player)}的${trigger.judgestr || ""}判定为${get.translation(trigger.player.judging[0])}，${get.prompt(event.skill)}`, "hes", (card) => {
        ...
        if (!player2.storage.nstaiping && get.color(card) != "black") {
          return false;
        }
  ...
  nsweiyuan: {
    ...
    direct: true,
    filter(event, player) {
      return player != event.target && event.targets && event.targets.length == 1 && event.target.isIn() && player.isPhaseUsing() && !player.hasSkill("nsweiyuan2") && game.hasPlayer(function(current) {
        return current != player && current != event.target;
      });
    },
    async content(event, trigger, player) {
      let result2;
      result2 = await player.chooseTarget(get.prompt2("nsweiyuan"), (card, player2, target2) => {
        return target2 != player2 && target2 != _status.event.getTrigger().target;
      }).set("ai", (target2) => {
        return Math.max(Math.random(), get.attitude(player, target2));
      }).forResult();
      if (!result2.bool) {
        return;
      }
      ...
      result2 = await target.chooseCard("he", "交给" + get.translation(trigger.target) + "一张牌并受到1点伤害，或令" + get.translation(player) + "摸一张牌且可以重复使用牌").set("ai", (card) => {
        if (_status.event.goon) {
          return Math.random();
        }
        return 0;
    ...
    chooseButton: {
      ...
      backup(links, player) {
        return {
          ...
          onuse(links2, player2) {
            player2.removeSkill("nsweiyuan_use");
          }
        };
      },
      prompt(links, player) {
  ...
  nszhenyin: {
    ...
    async content(event, trigger, player) {
      ...
      if (target?.hasCard((card) => {
        const player2 = _status.currentPhase;
        const mod2 = game.checkMod(card, player2, "unchanged", "cardEnabled2", player2);
        if (mod2 != "unchanged") {
          return mod2;
        }
  ...
  nsxianhai: {
    ...
    async content(event, trigger, player) {
      ...
      if (list.length) {
        result2 = await player.chooseControl(list).set("prompt", "选择废除" + get.translation(trigger.source) + "的一种装备栏").set("ai", () => {
  ...
  nsxingchu: {
    ...
    direct: true,
    async content(event, trigger, player) {
      const result2 = await player.chooseTarget(get.prompt2("nsxingchu")).set("ai", function(target) {
  ...
  nsdaizhan: {
    trigger: { player: "phaseZhunbeiBegin" },
    filter(event, player) {
      return ["lebu", "bingliang"].some((name) => player.hasCard((card) => player.canAddJudge({ name, cards: [card] }) && (_status.connectMode || get.type2(card) != "trick"), "he"));
    },
    async cost(event, trigger, player) {
      const list = ["lebu", "bingliang"].filter((name) => player.hasCard((card) => player.canAddJudge({ name, cards: [card] }) && get.type2(card) != "trick", "he"));
      const result2 = await player.chooseButton([get.prompt2(event.skill), [list.map((name) => [get.type(name), "", name]), "vcard"]]).set("ai", (button) => {
        const player2 = get.player();
        if (button.link[2] == "lebu") {
          return 0;
        }
        const delta = player2.getHandcardLimit() + player2.countCards("j") * 2 + 2 - player2.hp;
        if (delta >= 2) {
          return 1 + Math.random();
        }
        if (delta >= 0 && !player2.countCards("h", (card) => player2.hasValueTarget(card))) {
          return Math.random();
        }
        return 0;
      }).forResult();
    ...
    subSkill: {
      backup: {
        filterCard(card, player) {
          return get.itemtype(card) == "card" && get.type2(card) != "trick" && player.canAddJudge({
  ...
  nsjiquan: {
    ...
    async content(event, trigger, player) {
      ...
      event.target = target;
      result2 = await player.choosePlayerCard(target, "hej", player.storage.nsfuwei ? true : 1).set("ai", (button) => {
  ...
  nsdiemou: {
    ...
    async content(event, trigger, player) {
      const cards2 = player.getExpansions("nsjiquan_mark");
  ...
  junkshicai: {
    ...
    async content(event, trigger, player) {
      ...
      if (cards2.length > 1) {
        result2 = await player.chooseToMove({ prompt: "恃才：将牌按顺序置于牌堆顶" }).set("list", [["牌堆顶", cards2]]).set("reverse", _status.currentPhase?.next ? get.attitude(player, _status.currentPhase.next) > 0 : false).set("processAI", (list) => {
  ...
  junklangmie: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseControl("cancel2").set("choiceList", list).set("ai", function() {
        const player2 = _status.event.player;
      ...
      result2 = await player.chooseToDiscard("he").set("ai", (card) => 7 - get.value(card)).set("logSkill", event.choice == "draw" ? "junklangmie" : ["junklangmie", target]).forResult();
      if (result2.bool) {
  ...
  junkwangxi: {
    ...
    async content(event, trigger, player) {
      ...
      const result2 = await player.draw(2).forResult();
      if (get.itemtype(result2.cards) == "cards" && target.isIn() && player.hasCard((card) => result2.cards.includes(card), "he")) {
        await player.chooseToGive(target, "he", true, (card) => get.event().cards?.includes(card)).set("cards", result2.cards);
      }
    }
  },
  //2013标准包双蜀黑
  junkjizhi: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseCard("h", "是否用一张手牌交换" + get.translation(card) + "？", "若选择「取消」，则" + get.translation(card) + "将被置入弃牌堆。").forResult();
      if (result2.bool && result2.cards?.length) {
        const handcard = result2.cards[0];
        player.$throw(handcard, 1e3);
      ...
      while (player.countCards("h") > 0 && game.hasPlayer((current) => current != player && !assignedTargets.includes(current))) {
        result2 = await player.chooseCardTarget({
          ...
          filterTarget(card, player2, target2) {
            return target2 != player2 && !assignedTargets.includes(target2);
          }
        }).forResult();
        if (!result2.bool) {
          break;
      ...
      backup(links, player) {
        return {
          filterCard(card, player2) {
            return !player2.getStorage("junktaoluan2").includes(get.suit(card));
          },
          audio: "taoluan",
          popname: true,
          check(card) {
          ...
          onuse(result2, player2) {
            player2.markAuto("junktaoluan2", [get.suit(result2.cards[0], player2)]);
            ...
            player2.markAuto("junktaoluan", [result2.card.name]);
          }
        };
      },
      prompt(links, player) {
  ...
  junktaoluan2: {
    ...
    async content(event, trigger, player) {
      ...
      if (result2?.bool && result2.targets?.length) {
        ...
        result2 = await target.chooseCard("###滔乱###交给" + get.translation(player) + "一张不为" + get.translation(type) + "牌的牌，或令其失去1点体力且滔乱无效直到回合结束", "he", (card, player2, target2) => {
          ...
          return 0;
        }).set("att", get.attitude(target, player) > 0).forResult();
        if (result2?.bool && result2.cards?.length) {
          await target.give(result2.cards, player, "visible");
        } else {
  ...
  nsfengli: {
    ...
    async content(event, trigger, player) {
      const result2 = await player.chooseTarget(get.prompt2("nsfengli"), function(card, player2, target) {
        return target != player2 && !target.hasSkill("nsfengli_use");
      }).set("ai", function(target) {
    ...
    filter(event, player, name) {
      ...
      if (event.getg) {
        if (event.name == "gain") {
          ...
          player.checkHistory("gain", function(evt2) {
  ...
  nsfengli_clear: {
    ...
    async content(event, trigger, player) {
      const cards2 = player.getShownCards();
      if (cards2.length > 0) {
      ...
      backup(links, player) {
        ...
        return {
          ...
          async precontent(event, trigger, player2) {
            ...
            const target2 = player2.storage.nsfengli_use;
            event.target = target2;
            player2.logSkill("nsfengli", target2);
            await player2.showCards(card, get.translation(player2) + "发动了【奉礼】");
  ...
  nskaicheng: {
    ...
    async content(event, trigger, player) {
      const { target, cards: cards2 } = event;
      let result2;
      result2 = await target.chooseBool("是否将" + get.translation(cards2) + "告知" + get.translation(player)).set("ai", () => get.attitude(target, player) > 0).forResult();
      if (!player.hasUseTarget(cards2[0])) {
        if (result2.bool) {
          result2 = await player.chooseControl("确定").set("prompt", "你展示的手牌为" + get.translation(cards2)).forResult();
        } else {
          event.hidden = true;
          result2 = await player.chooseControl("确定").set("prompt", get.translation(target) + "拒绝告知你卡牌信息").forResult();
        }
      } else {
        if (result2.bool) {
          result2 = await player.chooseBool("是否使用展示的牌？", "你展示的手牌为" + get.translation(cards2) + "。如果你使用此牌，则在结算后摸一张牌；如果你不使用此牌，则结束出牌阶段").forResult();
        } else {
          event.hidden = true;
          result2 = await player.chooseBool("是否使用展示的牌？", get.translation(target) + "拒绝告知你卡牌信息。如果你使用此牌，则在结算后摸一张牌；如果你不使用此牌，则结束出牌阶段").forResult();
        }
      }
      if (result2.bool) {
        await player.chooseUseTarget(true, cards2[0], event.hidden ? "选择此牌的目标" : null);
      } else {
  ...
  nsjuanli: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseCard("h", true).set("prompt2", prompt2).forResult();
      if (!result2.bool) {
        return;
      }
      ...
      for (let i = 0; i < 1e3; i++) {
        ...
        if (suit == suit1) {
          await player.showCards(cards2, get.translation(player) + "赌牌获胜");
        ...
        if (suit == suit2) {
          await player.showCards(cards2, get.translation(target) + "赌牌获胜");
  ...
  nsbaiyi: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseToDiscard("白衣：请弃置" + get.cnNumber(num) + "张牌", "he", true, num).forResult();
      if (!result2?.bool || !result2.cards?.length) {
        return;
      }
      let goon = true;
      ...
      result2 = await player.chooseCardButton(cards2, "白衣：获得其中一张牌", true).forResult();
      if (result2?.bool && result2.links?.length) {
        await player.gain(result2.links, "draw");
        const discard = cards2.filter((card) => !result2.links.includes(card));
        if (discard.length) {
          player.$throw(discard, 1e3);
  ...
  nsbaiming: {
    ...
    async cost(event, trigger, player) {
      const skills2 = player.storage.nsbaiming.slice(0);
      ...
      const { bool, links } = await player.chooseButton([`###百鸣###<div class="text center">你可以获得其中一个技能</div>`, [list, "textbutton"]]).set("displayIndex", false).set("ai", (button) => {
  ...
  nstanbing: {
    ...
    async content(event, trigger, player) {
      let result2;
      result2 = await player.chooseToDiscard("h", get.prompt2("nstanbing")).set("ai", (card) => {
      ...
      if (result2.bool) {
        await player.draw(get.translation(result2.cards[0].name).length);
      ...
      damage: {
        ...
        async content(event, trigger, player) {
          ...
          if (player.hasCards("he")) {
            result2 = await target.discardPlayerCard(player, "混击", "he").set("boolline", true).set("prompt2", "弃置" + get.translation(player) + "的一张牌，或取消并摸一张牌").forResult();
          } else {
            await target.draw();
            return;
          }
      ...
      use: {
        ...
        content: [
          async (event, trigger, player) => {
            ...
            event.cardx = card;
            await player.showCards(get.translation(player) + "亮出了" + get.translation(card), cards2).set("hiddencards", cards22);
    ...
    subSkill: {
      bugua: {
        ...
        async content(event, trigger, player) {
          ...
          result2 = await player.chooseToDiscard({
            prompt: "推演：是否发动一次【卜卦】？",
            position: "he",
            ai(card) {
              return 8 - get.value(card);
  ...
  nscangxi2: {
    ...
    async content(event, trigger, player) {
      ...
      while (list.length) {
        ...
        let result2 = await player.chooseBool(get.prompt("nscangxi", current)).set("choice", get.attitude(player, current) > 0).forResult();
        if (!result2.bool) {
          continue;
        }
        player.logSkill("nscangxi", current);
        result2 = await player.judge((card) => {
          return get.event().att * (get.color(card) == "black" ? 1 : 0);
        }).set("att", get.sgnAttitude(player, current)).forResult();
        ...
        result2 = await player.chooseControlList(["令" + name + "摸一张牌展示", "令" + name + "手牌上永久+1", "弃置一张牌并令" + name + "获得一张本回合进入弃牌堆的牌"], () => {
        ...
        if (result2.index == 2) {
          result2 = await player.chooseToDiscard(true, "he").forResult();
          if (!result2.bool) {
            continue;
          }
  ...
  nswulie: {
    ...
    async content(event, trigger, player) {
      ...
      player.awakenSkill(event.name);
      await player.loseMaxHp();
      result2 = await player.chooseCardButton(Array.from(ui.discardPile.childNodes), "将至多三张任意顺置于牌堆顶（先选择的在上）", true, [1, 3]).forResult();
      if (result2.bool) {
    ...
    subSkill: {
        ...
        async content(event, trigger, player) {
          ...
          result2 = await player.chooseCardButton(choices, "将至多三张任意顺置于牌堆顶（先选择的在上）", true, [1, 3]).forResult();
          if (result2.bool) {
  ...
  nshunyou: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseTarget("是否将" + get.translation(event.equip) + "装备给一其角色？", (card, player2, target2) => {
  ...
  nsgongjian: {
    ...
    async content(event, trigger, player) {
      let result2;
      result2 = await player.chooseTarget("恭俭：将弃置的牌交给一名体力值大于你的角色", (card, player2, target) => {
  ...
  nscaijian: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseCard("将" + get.cnNumber(num) + "张手牌以按顺序置于牌堆顶（先选择的在上）", num, true).set("ai", function(card) {
        return -get.value(card);
      }).forResult();
      if (!result2.bool) {
        return;
      }
      const next = player.lose(result2.cards, ui.special);
  ...
  nsxionglue: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseButton(dialog, true).set("ai", (button) => {
  ...
  nshuanhuo: {
    ...
    async content(event, trigger, player) {
      let result2;
      result2 = await player.chooseTarget(2, get.prompt2("nshuanhuo"), function(card, p, target) {
      ...
      if (result2.bool) {
        player.logSkill("nshuanhuo", result2.targets);
      } else {
        event.finish();
        return;
      }
  ...
  nsyaowang: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (current) => player != current && current.getSkills(null, false, false).filter((skill) => {
          const info = get.info(skill);
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget(get.prompt2(event.skill), (card, player2, target) => {
        return player2 != target && target.getSkills(null, false, false).filter((skill) => {
          const info = get.info(skill);
          return info && !info.charlotte;
        }).length;
      }).set("ai", (target) => {
    ...
    async content(event, trigger, player) {
      ...
      const { links } = await player.chooseButton(["选择获得一个技能", [list, "textbutton"]]).set("displayIndex", false).set("ai", (button) => {
        get.player();
  ...
  nsyunxing: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseTarget("弃置一名角色的一张手牌", true, function(card, player2, target) {
        return target.countCards("h");
      }).set("ai", function(target) {
        if (target.hasSkillTag("noh")) {
          return 0;
    ...
    subSkill: {
      self: {
        ...
        async content(event, trigger, player) {
          let result2;
          result2 = await player.chooseTarget(get.prompt("nsyunxing"), (card, _player, target) => {
            return target != player;
          ...
          if (result2.bool) {
            player.logSkill("nsyunxing", result2.targets);
  ...
  nsguanxing: {
    ...
    async content(event, trigger, player) {
      ...
      let num = player.hp;
      while (num > 0) {
        const judgeCards2 = player.getCards("j");
        let pos = 0;
        let choiceIndex = -1;
        ...
        result2 = await player.chooseCardButton(`观星：选择要移动的牌（还能移动${num}张）`, cards2).set("filterButton", (button) => {
        ...
        chosen.push(card);
        cards2.remove(card);
        result2 = await player.chooseControl(() => {
    ...
    async content(event, trigger, player) {
      ...
      while (targets.length) {
        ...
        if (target.isAlive() && current.countCards("he")) {
          const result2 = await current.chooseToUse({ name: "sha" }, target, -1, "号令").set("prompt2", "选择一项：1. 对" + get.translation(target) + "使用一张【杀】；2. 取消并交给" + get.translation(player) + "一张牌，然后其视为对你使用一张【杀】").forResult();
          if (result2.bool) {
            continue;
          }
          if (current.countCards("he")) {
            const { cards: cards2 } = await current.chooseCard("he", true, "交给" + get.translation(player) + "一张牌").forResult();
            if (cards2) {
              await current.give(cards2, player);
            }
          }
  ...
  nsduijue: {
    ...
    async content(event, trigger, player) {
      ...
      let result2;
      result2 = await player.chooseToDiscard(get.prompt2("nsduijue")).set("ai", function(card) {
      ...
      if (result2.bool) {
        ...
        player.storage.nsduijue_use = get.color(result2.cards[0]);
      }
    },
    subSkill: {
      use: {
  ...
  nstaiping_nh: {
    ...
    async cost(event, trigger, player) {
      ...
      const result2 = await player.chooseControlList(list, () => get.event().choice).set("prompt", get.prompt(event.skill)).set("choice", choice).forResult();
      event.result = {
      ...
      die: {
        ...
        async content(event, trigger, player) {
          ...
          if (left && player.hasSkill(left) && right && player.hasSkill(right)) {
            ...
            const { control } = await player.chooseControl("releiji", "guidao").set("prompt", `令${get.translation(target)}获得一项技能`).forResult();
      ...
      right: {
        ...
        async content_old(event, trigger, player) {
          ...
          if (controls.length > 1) {
            result2 = await player.chooseControl(controls).set("prompt", "选择一个形态直到下一回合开始").set("ai", () => Math.floor(Math.random() * _status.event.num)).set("num", controls.length).forResult();
          } else {
            return;
          }
  ...
  nsdufu: {
    ...
    async content(event, trigger, player) {
      let result2;
      result2 = await player.chooseTarget(get.prompt2("nsdufu"), (card, player2, target) => {
        return target != player2;
      }).set("ai", (target) => {
      ...
      if (result2.bool && result2.targets?.length) {
        player.logSkill("nsdufu", result2.targets);
  ...
  liangji: {
    ...
    async content(event, trigger, player) {
      const { target } = event;
      const result2 = await player.chooseCard("h", "环计：将一张牌置于" + get.translation(target) + "的武将牌上", true).set("ai", function(card) {
      ...
      if (result2.bool) {
        ...
        await player.lose(result2.cards, ui.special);
    ...
    subSkill: {
      1: {
        ...
        async content(event, trigger, player) {
          const cards2 = player.storage.liangji_1;
          if (cards2) {
            await player.gain(cards2, "gain2");
          }
  ...
  jugong: {
    ...
    async content(event, trigger, player) {
      ...
      const result2 = await player.chooseCard("将" + get.cnNumber(1) + "张手牌置于武将牌上作为”功”", 1, true).forResult();
      if (result2.cards && result2.cards.length) {
        await player.lose(result2.cards, ui.special);
        player.storage.jugong = player.storage.jugong.concat(result2.cards);
        player.syncStorage("jugong");
        player.markSkill("jugong");
        game.log(player, "将", result2.cards, "置于武将牌上作为”功”");
      }
    },
    intro: {
      content: "cards"
    ...
    subSkill: {
      1: {
        ...
        async content(event, trigger, player) {
          let result2;
          result2 = await player.chooseCardButton("移去两张“功”", 2, player.storage.jugong, true).forResult();
  ...
  chengmou: {
    ...
    async content(event, trigger, player) {
      ...
      const cards2 = player.storage.jugong;
      if (cards2) {
        await player.gain(cards2, "gain2");
      }
      player.storage.jugong = [];
  ...
  nsdingzhou: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.draw().forResult();
      if (Array.isArray(result2?.cards) && get.color(card) != get.color(result2.cards[0])) {
        await player.loseHp();
      }
    },
    ai: {
  ...
  junkyuheng: {
    ...
    async content(event, trigger, player) {
      ...
      result2 = await player.chooseToDiscard({
        ...
        filterCard(card, player2) {
          ...
          for (const selectedCard of ui.selected.cards) {
            if (get.suit(selectedCard, player2) == suit) {
              return false;
            }
          }
          return true;
      ...
      if (result2.bool) {
        ...
        await player.addAdditionalSkills("junkyuheng", skills2, true);
      }
    },
  ...
  junkdili: {
    ...
    filter(event, player) {
      ...
      return skills2.length > player.maxHp;
    },
    async content(event, trigger, player) {
      let result2;
      player.awakenSkill(event.name);
      ...
      result2 = await player.chooseButton(["请选择失去任意个技能", [skills2, "skill"]]).set("forced", true).set("selectButton", [1, skills2.length]).set("skills", skills2).set("ai", (button) => {
  ...
  nsmanzhi: {
    ...
    async content(event, trigger, player) {
      ...
      const result2 = await player.chooseButton(2, [
        "蛮智：请选择让下列等式成立的A与B的值",
...
const translates = {
  ...
  nshunji: "混击",
  nshunji_info: "出牌阶段限一次，你可以摸一张牌，视为使用一张【万箭齐发】。此【万箭齐发】造成伤害时，受伤角色选择一项：①弃置你一张牌；②摸一张牌。",
  nscuanquan: "篡权",
...
const dynamicTranslates = {
  ...
  abyusa_jueqing(player) {
    if (player.storage.abyusa_jueqing_rewrite) {
  ...
  shiroha_guying(player) {
    ...
    if (!player.storage.shiroha_jiezhao) {
      str = "锁定技，每回合限一次，" + str;
    }
    if (player.storage.shiroha_jiezhao) {
      str += "可";
