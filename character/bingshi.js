...
const skills = {
  ...
  potheyun: {
    ...
    usable: 2,
    filter(event, player) {
      const skills2 = player.getSkills(null, false, false).filter((skill2) => {
        const info = get.info(skill2);
        const dizhuSkill = ["飞扬", "跋扈", "恃强", "殷富"];
        const t = lib.translate[skill2];
        if (dizhuSkill.includes(t)) {
          return true;
        }
        return info && !info.charlotte;
      });
      return skills2.length && game.hasPlayer((current) => get.info("potheyun").filterTarget(null, player, current));
    },
    filterTarget(card, player, target) {
      ...
      return target.getSkills(null, false, false).containsSome(...skills2);
    },
    async content(event, trigger, player) {
      ...
      for (const skill2 of skills2) {
        list.push([skill2, `<div class="popup text" style="width:calc(100% - 10px);display:inline-block"><div class="skill">【` + get.translation(skill2) + "】</div><div>" + lib.translate[skill2 + "_info"] + "</div></div>"]);
      }
      const result = skills2.length > 1 ? await player.chooseButton({
        createDialog: ["合韵：请失去一个技能", [list, "textbutton"]],
        forced: true,
        ai(button) {
          const player2 = get.player();
          if (player2.additionalSkills?.potyinhui?.includes(button.link)) return 10 + Math.random();
          if (get.info(skill).ai?.neg) return 100 + Math.random();
          return 1 + Math.random();
        }
      }).forResult() : { bool: true, links: skills2 };
      if (result?.bool && result.links?.length) {
        const skills3 = result.links;
        await player.removeSkills(skills3);
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          ...
          return skills2.some((skill2) => {
            if (player.additionalSkills?.potyinhui?.includes(skill2)) return true;
            return get.info(skill2).ai?.neg;
          }) ? 1 : -1;
        },
        target: 1
      }
  ...
  potyinhui: {
    ...
    filter(event, player) {
      return game.hasPlayer((target) => {
        ...
        return target.getSkills(null, false, false).filter((skill2) => {
          if (player.hasSkill(skill2)) return false;
          const info = get.info(skill2);
          const dizhuSkill = ["飞扬", "跋扈", "恃强", "殷富"];
          const t = lib.translate[skill2];
          if (dizhuSkill.includes(t)) {
    ...
    //参考朱佩兰
    refreshSkill(player, skills2) {
      if (typeof skills2 === "string") skills2 = [skills2];
      Object.keys(player.storage).filter((i) => skills2.some((skill2) => i.startsWith(skill2))).forEach((storage) => delete player.storage[storage]);
      const suffixs = ["used", "round", "block", "blocker"];
      for (const skill2 of skills2) {
        const info = get.info(skill2);
        if (info.usable !== void 0) {
          if (typeof player.getStat("triggerSkill")[skill2] == "number" && player.getStat("triggerSkill")[skill2] >= 1) {
            delete player.getStat("triggerSkill")[skill2];
          }
          if (typeof player.getStat("skill")[skill2] == "number" && player.getStat("skill")[skill2] >= 1) {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        filterTarget(card, player2, target) {
          ...
          return target.getSkills(null, false, false).filter((skill2) => {
            if (player2.hasSkill(skill2)) return false;
            const info = get.info(skill2);
            const dizhuSkill = ["飞扬", "跋扈", "恃强", "殷富"];
            const t = lib.translate[skill2];
            if (dizhuSkill.includes(t)) {
        ...
        ai(target) {
          const { player: player2, filterTarget } = get.event();
          const targets = game.filterPlayer((target2) => filterTarget(null, player2, target2));
          return Math.max(
            ...targets.map((target2) => {
              ...
              return Math.max(...skills2.map((skill2) => get.skillRank(skill2)));
            })
          );
        }
      }).forResult();
    ...
    async content(event, trigger, player) {
      ...
      const skills2 = target.getSkills(null, false, false).removeArray(player.getSkills(null, false, false)).filter((skill2) => {
        const info = get.info(skill2);
        const dizhuSkill = ["飞扬", "跋扈", "恃强", "殷富"];
        const t = lib.translate[skill2];
        if (dizhuSkill.includes(t)) {
      ...
      if (skills2.length) {
        ...
        if (result?.bool && result.links?.length) {
          const skill2 = result.links[0];
          player.logSkill(event.name, null, null, null, [get.rand(3, 12)]);
          get.info("potyinhui").refreshSkill(player, skill2);
          await player.addAdditionalSkills(event.name, skill2, true);
        }
      }
    }
  },
  ...
  potchiyun: {
    ...
    filter(event, player) {
      ...
      return event.getg?.(player)?.length && lib.phaseName.some((phase) => {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseCardTarget({
        ...
        ai2(target) {
          const player2 = get.player();
          if (player2.storage.potchiyun1 && game.hasPlayer((current) => get.attitude(player2, current) > 0 && player2 != current)) {
    ...
    async content(event, trigger, player) {
      ...
      const colors = cards2.map((card) => get.color(card)).unique();
      await player.give(cards2, target);
      const list = [], choiceList = ["展示所有" + get.translation(colors) + "手牌并受到来自" + get.translation(player) + "的一点火焰伤害", "令" + get.translation(player) + "摸两张牌，然后你进入连环状态"];
      ...
      const result = list.length > 1 ? await target.chooseControl(list).set("choiceList", choiceList).set("prompt", "请选择一项").set("ai", () => get.event().controls.randomGet()).forResult() : { control: list[0] };
  ...
  potyanhui: {
    ...
    async cost(event, trigger, player) {
      ...
      }).set("targets", trigger.targets.filter((target) => target.hasCards("h"))).forResult();
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
      ...
      if (result?.bool && result.links?.length) {
        ...
        await target.showCards(cards2, `${get.translation(player)}发动了【${get.translation(event.name)}】`);
        if (shown.includes(cards2[0])) {
          await target.modedDiscard({ cards: cards2, discarder: player });
        }
      }
      if (!player.hasSkill(event.name + "_used")) {
        player.addTempSkill("potyanhui_used", "phaseAnyAfter");
        player.when({ global: "phaseAnyEnd" }).step(async (event2, trigger2, player2) => {
          const bool1 = game.hasPlayer((current) => {
            return current.hasHistory("lose", (evt) => evt.type == "discard" && evt.getParent(2).name == "potyanhui" && evt.getParent(trigger2.name) == event2.getParent(trigger2.name));
          }) && !player2.storage.potyanhui1, bool2 = game.hasPlayer2((current) => {
            return game.hasGlobalHistory("everything", (evt) => {
              return evt.name == "showCards" && evt.cards?.length > 0 && evt.player == current;
            });
          }) && !player2.storage.potyanhui2;
          if (!bool1 && !bool2) {
            return;
          }
          ...
          const result2 = await player2.chooseButtonTarget({
            ...
            forced: true,
            filterButton(button) {
              const { player: player3, targets: targets2, num: num2 } = get.event();
              if (button.link == "damage") {
                return targets2.length > 0 && !player3.storage.potyanhui1;
              }
              return num2 > 0 && !player3.storage.potyanhui2;
            },
            ...
            ai1(button) {
              const { player: player3, targets: targets2, num: num2 } = get.event();
              if (button.link == "draw") {
                if (player3.isPhaseUsing() && player3.needsToDiscard()) {
                  return 0;
                }
                return num2 * get.effect(player3, { name: "draw" }, player3, player3);
              }
              return Math.max(
                ...targets2.map((current) => {
                  return get.damageEffect(current, player3, player3, "fire");
                })
              );
            },
            ai2(target2) {
          ...
          }).set("num", num).set("targets", targets).forResult();
          if (result2?.bool && result2.links?.length) {
            ...
            if (link == "damage") {
              if (target2?.isIn()) {
                ...
                player2.logSkill("potyanhui", [target2], null, null, [player2, get.rand(5, 6)]);
                await target2.damage("fire");
              }
            } else {
              player2.logSkill("potyanhui", null, null, null, [player2, get.rand(3, 4)]);
    ...
    init(player, skill2) {
      player.addSkill(skill2 + "_mark");
      game.countPlayer((current) => {
        const cards2 = current.getCards("h", (card) => {
          const shown = game.getGlobalHistory("everything", (evt) => {
            return evt.name == "showCards" && evt.cards?.length > 0;
          }).map((evt) => evt.cards).flat();
        ...
        if (cards2.length) {
          current.addGaintag(cards2, skill2);
        }
      });
    },
    onremove(player, skill2) {
      player.removeSkill(skill2 + "_mark");
      game.countPlayer((current) => current.removeGaintag(skill2));
    },
    subSkill: {
      used: { charlotte: true },
      mark: {
        ...
        async content(event, trigger, player) {
          ...
          if (trigger.name == "phase") {
            ...
            game.broadcastAll((cards3) => {
              cards3.forEach((card) => card.addGaintag("potyanhui"));
            }, cards2);
          } else {
            const cards2 = target.getCards("h", (card) => {
              const shown = game.getGlobalHistory("everything", (evt) => {
                return evt.name == "showCards" && evt.cards?.length > 0;
              }).map((evt) => evt.cards).flat();
              return shown.includes(card);
            ...
            if (cards2.length) {
              target.addGaintag(cards2, "potyanhui");
            }
          }
        }
      }
  ...
  potfentao: {
    ...
    logTarget: "player",
    async content(event, trigger, player) {
      const target = event.targets[0], bool1 = player.storage[event.name + "1"], bool2 = player.storage[event.name + "2"];
      if (bool1 && bool2) {
        return;
      }
      if (target?.isIn()) {
        ...
        if (bool2) {
          ...
          result = target.hasDiscardableCards(target, "he") ? await target.chooseToDiscard({
            forced,
            prompt: `${get.translation(player)}对你发动了焚涛：请选择弃置一半牌` + (bool1 ? "" : "或点击“取消”令本次传导伤害+1"),
            position: "he",
            selectCard: Math.ceil(target.countCards("he") / 2),
            ai(card) {
              const { player: player2, num } = get.event();
              if (num + 1 >= player2.getHp() + player2.hujia) {
                return 114514 - get.value(card);
              }
              return 7 - get.value(card);
            }
          }).set("num", trigger.num).forResult() : { bool: false };
        }
        if (!result?.bool || !result.cards?.length) {
          ...
          target.when({ player: "damageAfter" }).filter((evt) => evt === trigger).step(async (event2, trigger2, player2) => {
            player.logSkill("potfentao", target, null, null, [event2, player]);
  ...
  potxiongzi: {
    ...
    animationColor: "fire",
    filter(event, player) {
      return ["potchiyun", "potyanhui", "potfentao"].some((skill2) => player.hasSkill(skill2, null, null, false));
    },
    async cost(event, trigger, player) {
    ...
    async content(event, trigger, player) {
      ...
      for (const skill2 of ["potchiyun", "potyanhui", "potfentao"]) {
        player.setStorage(skill2 + (link === "保留选项一" ? "2" : "1"), true, true);
      }
      await player.draw({ num: 2 });
    }
  },
  ...
  potduzuo: {
    ...
    async content(event, trigger, player) {
      const target = event.targets[0];
      const card = get.cardPile((card2) => get.name(card2) == "sha" && get.nature(card2) == "fire");
      if (card) {
  ...
  potbihan: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player.chooseTarget({
        ...
        filterTarget(card, player2, target2) {
          ...
          return [targetx, player2].includes(target2) && target2.countCards("h") > target2.getHp();
        },
        ai(target2) {
          const { player: player2, targetx } = get.event();
          if (get.attitude(player2, targetx) > 0 && [player2, targetx].some((i) => i.countCards("h") - i.getHp() <= 2)) {
            return get.attitude(player2, target2) * (114514 - target2.countCards("h"));
          } else {
    ...
    async content(event, trigger, player) {
      ...
      if (target.hasDiscardableCards(target, "h")) {
        const { cards: cards2 } = await target.chooseToDiscard({ forced: true, position: "h", selectCard: num }).forResult();
  ...
  potlibing: {
    ...
    intro: {
      ...
      content(storage, player) {
        ...
        if (!storage) {
          ...
          const num2 = 1 + player.countMark("potlibing_draw");
          str += "摸" + num2 + "张牌";
    ...
    onremove(player, skill2) {
      player.removeSkill(skill2 + "_attack");
      player.removeSkill(skill2 + "_draw");
  ...
  potpoxi: {
    ...
    getIndex(event, player) {
      const evt = event.getl(player), cards2 = evt.cards2.filter((card) => card.name == "sha" && player.hasUseTarget(card, false, false));
      return cards2;
    },
    direct: true,
    async content(event, trigger, player) {
      ...
      if (player.hasUseTarget(card, false, false)) {
        await player.chooseUseTarget({
          ...
          event2.directHit.addArray(targets);
  ...
  potdingpin: {
    ...
    async cost(event, trigger, player) {
      ...
      for (const target of game.filterPlayer()) {
        const lose = target.iterHistory("lose", (evt) => evt.cards2?.length).map((evt) => evt.cards2.length).reduce((sum, len) => sum + len, 0);
        if (lose > maxLose) {
          maxLose = lose;
          targets = [target];
  ...
  potsifeng: {
    ...
    async content(event, trigger, player) {
      ...
      if (!cards2.length || !game.hasPlayer((current) => current != player)) {
        return;
      }
    ...
    intro: {
      ...
      mark(dialog, content, player) {
        const cards2 = player.getExpansions("potsifeng");
        if (cards2.length) {
          if (player.isUnderControl(true)) {
            dialog.addAuto(cards2);
          } else {
            return "共有" + get.cnNumber(cards2.length) + "张“伺锋”";
          }
        }
      },
    ...
    subSkill: {
      ...
      effect: {
        ...
        async cost(event, trigger, player) {
          ...
          const result = await player.chooseControl({
            ...
            ai() {
              ...
              const cards2 = target2.getExpansions("potsifeng");
              if (get.attitude(player2, target2) > 0) {
                return "选项一";
              } else {
                if (cards2.length >= 3) {
        ...
        async content(event, trigger, player) {
          const target = event.targets[0], cards2 = target.getExpansions("potsifeng");
          if (event.cost_data == 0) {
            ...
            if (cards2.length) {
              await player.gain({ cards: cards2, animate: "give" });
            }
          }
        }
      }
  ...
  pothuilv: {
    ...
    async cost(event, trigger, player) {
      const list = ["red", "black", "cancel2"].removeArray(player.getStorage(event.skill + "_round")), target = trigger.player;
      const result = await player.chooseControl({
        controls: list,
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          if (bool2) {
            ...
            const cards2 = player.getStorage(event.name).filter((card) => get.position(card) !== "d");
  ...
  potsongyan: {
    ...
    getColors(player) {
      const colors = player.getRoundHistory("lose", (evt) => evt.type == "discard").flatMap((evt) => evt.cards).map((card) => get.color(card)).unique();
      return colors;
    },
    subSkill: {
      wuxie: {
  ...
  potshishi: {
    getRespondEvts(event, player) {
      let respondEvts = [];
      for (const current of game.filterPlayer2((current2) => current2 != player, null, true)) {
    ...
    filter(event, player) {
      ...
      if (player.hasAllHistory("useCard", (evt) => {
    ...
    logTarget(event, player) {
      return player.getAllHistory("damage", (evt) => evt.source?.isIn()).map((evt) => evt.source).toUniqued().sortBySeat();
    },
    async content(event, trigger, player) {
  ...
  potdianyi: {
    ...
    async content(event, trigger, player) {
      ...
      if (gain.length) {
        await game.loseAsync({
        ...
        }).setContent(async (event2) => {
          event2.type = "gain";
          const { cards: cards2, gain_list } = event2;
          const position = [];
          const [[player2]] = gain_list;
          ...
          for (const index in position) {
            ...
            if ("hesx".includes(pos)) {
              ...
              owner?.$giveAuto([card], player2);
            } else {
              player2.$gain2([card], true);
            }
          }
          await game.delay(0, get.delayx(500, 500));
          await player2.gain({ cards: cards2 }).set("getlx", false);
          await game.delayx();
        });
      }
  ...
  potjianzhuan: {
    ...
    async content(event, trigger, player) {
      ...
      const choices = ["potdianyi", "potshequan"].filter((skill2) => player.hasSkill(skill2, null, false, false)).map((skill2) => get.translation(skill2));
      const { triggers: potdianyi } = player.getStorage("potdianyi", { triggers: [] });
      const { triggers: potshequan } = player.getStorage("potshequan", { triggers: [] });
      if (potdianyi.concat(potshequan).length == 3) {
        ...
        const result = await player.chooseControl({
          ...
          ai(event2) {
            ...
            return event2.triggername == "dying" ? 0 : 1;
          }
        }).forResult();
  ...
  potnizun: {
    onremove(player, skill2) {
      const cards2 = player.getExpansions(skill2);
      if (cards2.length) {
    ...
    intro: {
      ...
      mark(dialog, storage, player) {
        ...
        const cards2 = player.getExpansions("potnizun");
        if (cards2.length > 0) {
    ...
    filter(event, player) {
      ...
      return cards2.some((card) => event.filterCard(card, player, event));
    },
    chooseButton: {
      dialog(event, player) {
        const cards2 = player.getExpansions("potnizun");
    ...
    async content(event, trigger, player) {
      ...
      if (suits.length) {
        ...
        if (result.control) {
          ...
          player.addTip(
            event.name,
            `${get.translation(event.name)} ${player.getStorage(event.name).map((i) => get.translation(i)).join("")}`
          );
        }
      }
    },
      ...
      sha: {
        ...
        async cost(event, trigger, player) {
          ...
          event.result = await player.chooseTarget({
            ...
            filterTarget(card, player2, target) {
              return player2.canUse(card, target, false, false);
            },
            ai(target) {
        ...
        async content(event, trigger, player) {
          ...
          if (!storage.length) {
            ...
            player.addTip(
              name,
              `${get.translation(name)} ${player.getStorage(name).map((i) => get.translation(i)).join("")}`
            );
          }
  ...
  potnigu: {
    ...
    chooseToGive(player, source, eventId) {
      const next = player.chooseCard({
        ...
        position: "he",
        ai(card) {
          const { sourcex, player: player2 } = get.event();
          return get.attitude(player2, sourcex) > 0 ? sourcex.getUseValue(card) : 6 - get.value(card);
        }
      });
  ...
  potlulian: {
    ...
    filter(event, player) {
      ...
      return player.hasHistory("lose", (evt) => (evt.relatedEvent || evt.getParent()) == event && evt.hs?.length) && !player.hasCards("h", (card) => get.type2(card) == type) && event.targets?.some((target) => {
    ...
    subSkill: {
      phaseUse: {
        ...
        async precontent(event, trigger, player) {
          const skill2 = event.name.slice(4);
          const result = await player.chooseNumbers(`###${get.translation(skill2)}###出牌阶段限一次，你可以消耗任意点蓄力值，令至多等量名角色从牌堆或弃牌堆中各获得一张红桃牌`, [{ prompt: "请选择要移去的蓄力值", min: 1, max: player.countCharge() }]).set("processAI", () => {
            const player2 = get.player();
          ...
          if (result?.bool && result.numbers?.length) {
            ...
            event.getParent().set(skill2, result.numbers[0]);
          } else {
  ...
  mbsizi: {
    ...
    chargeSkill: 4,
    init(player, skill2) {
      const num = lib.skill[skill2].beginMarkCount;
      ...
      backup(result, player) {
        return {
          ...
          async content(event, trigger, player2) {
            const { control: num } = get.info(event.name), skill2 = "mbsizi_effect";
            player2.removeCharge(num);
            player2.addTempSkill(skill2, { player: "phaseBegin" });
            player2.addMark(skill2, num, false);
  ...
  mbyunan: {
    ...
    async content(event, trigger, player) {
      ...
      player.awakenSkill(event.name);
      const skill2 = "mbkechang";
      if (!player.hasSkill(skill2, null, null, false)) {
        await player.addSkills(skill2);
      } else {
        player.setStorage(skill2, true);
        player.popup(skill2, "purple");
        game.log(player, "升级了技能", `#g【${get.translation(skill2)}】`);
        await game.delayx();
      }
    },
        ...
        async content(event, trigger, player) {
          ...
          if (result?.links?.length) {
            ...
            if (player.hasCard((card) => get.suit(card, player) == choice, "h")) {
              const skill2 = "potjiejie_effect";
              player.markAuto(skill2, [choice]);
              player.addTip(
                skill2,
                `诫节${player.getStorage(skill2).sort((a, b) => lib.suit.indexOf(b) - lib.suit.indexOf(a)).map((suit) => get.translation(suit)).join("")}`
              );
  ...
  pothaoshi: {
    ...
    async content(event, trigger, player) {
      const target = event.targets[0], skill2 = event.name + "_clear";
      target.markAuto(event.name + "_use", player);
      target.addAdditionalSkill(`${event.name}_use_${player.playerid}`, event.name + "_use");
      player.addTempSkill(skill2, { player: "phaseBegin" });
      player.storage[skill2].set(target, 2);
      player.addTip(
        skill2,
        [...player.storage[skill2].entries()].map(([target2, num]) => {
          return `${get.translation(skill2)}${get.translation(target2)} ${num}`;
        }).join("<br>")
      );
    ...
    subSkill: {
      ...
      draw: {
        ...
        async content(event, trigger, player) {
          const skill2 = "pothaoshi_clear";
          player.storage[skill2].set(trigger.getParent().pothaoshi, player.storage[skill2].get(trigger.getParent().pothaoshi) - 1);
          player.addTip(
            skill2,
            [...player.storage[skill2].entries()].map(([target, num]) => {
              return `${get.translation(skill2)}${get.translation(target)} ${num}`;
            }).join("<br>")
          );
      ...
      clear: {
        charlotte: true,
        init(player, skill2) {
          player.storage[skill2] = /* @__PURE__ */ new Map([]);
        },
        onremove(player, skill2) {
          [...player.storage[skill2].entries()].forEach(([target, num]) => {
      ...
      use: {
        init(player, skill2) {
          ...
          const cards2 = player.getStorage(skill2).reduce((cards3, target) => {
  ...
  mbxiongtu: {
    ...
    usable(skill2, player) {
      return player.hasSkill(skill2 + "_double") ? 2 : 1;
    },
    filter(event, player) {
    ...
    async content(event, trigger, player) {
      ...
      if (result?.cards?.length) {
        ...
        const resultx = await player.chooseToDiscard({
  ...
  mbxianshuai: {
    audio: 2,
    init(player, skill2) {
      player.addSkill(skill2 + "_record");
    },
    onremove(player, skill2) {
      player.removeSkill(skill2 + "_record");
    },
    mod: {
      cardUsable(card, player, num) {
        if (_status.currentPhase != player) {
      ...
      record: {
        init(player, skill2) {
          ...
          if (suits.length) {
            ...
            player.addTip(skill2, `${get.translation(skill2)} ${player.getStorage(skill2).reduce((str, suit) => str += get.translation(suit), "")}`);
          }
        },
  ...
  potyinzhan: {
    ...
    async content(event, trigger, player) {
      const target = trigger.player, bool1 = target.hp >= player.hp, bool2 = target.countCards("h") >= player.countCards("h");
  ...
  mbfeijing: {
    ...
    async content(event, trigger, player) {
      ...
      if (targets.length) {
        ...
        await player.showCards(cards2, get.translation(player) + "发动了【飞径】").set("customButton", (button) => {
  ...
  mbxiugeng: {
    ...
    subSkill: {
      effect: {
        ...
        popup: false,
        init(player, skill2) {
          const storage = player.storage[skill2];
          if (storage >= 0) {
            player.addTip(skill2, `${get.translation(skill2)} ${storage}`);
          }
        },
        onremove(player, skill2) {
          delete player.storage[skill2];
    ...
    subSkill: {
      discard: {
        ...
        async content(event, trigger, player) {
          ...
          player.removeSkill(event.name);
          player.when({ player: "useCardAfter" }).filter((evt) => evt == trigger).step(async () => {
    ...
    chooseButton: {
      ...
      backup(result, player) {
        return {
          ...
          delay: false,
          async content(event, trigger, player2) {
            const num = lib.skill.mbrunwei_backup.num, skill2 = "mbrunwei";
            const cards2 = get.cards(num, true);
            player2.logSkill("mbrunwei", null, null, null, [get.rand(1, 2)]);
            await player2.showCards(cards2, `${get.translation(player2)}发动了【${get.translation(skill2)}】`);
            const used = player2.hasSkill(skill2 + "_twice");
            ...
            if (result2?.links?.length && result2?.targets.length) {
              ...
              player2.line(target);
              if (!player2.hasSkill(skill2 + "_twice")) {
                player2.addTempSkill(skill2 + "_twice", "phaseChange");
                player2.addMark(skill2 + "_twice", gain.length, false);
                player2.addTip(skill2 + "_twice", `润微  ${gain.length}`);
              }
              let gaintag = [];
              if (player2 == target) {
                gaintag = ["mbrunwei"];
  ...
  mbshuanghuai: {
    ...
    init(player, skill2) {
      const history = player.getAllHistory("useSkill", (evt) => evt.skill == skill2 && evt.targets);
      if (history.length) {
        const target = history[history.length - 1].targets[0];
        if (target) {
          player.storage[skill2] = target;
          player.markSkill(skill2);
          player.addTip(skill2, `霜怀 ${get.translation(target)}`);
        }
      }
    },
    onremove(player, skill2) {
      ...
      effect: {
        ...
        onremove(player, skill2) {
          game.filterPlayer((target) => {
            return player.storage[skill2].includes(target);
    ...
    subSkill: {
      ...
      guanshi: {
        ...
        async content(event, trigger, player) {
          ...
          const result = await target.chooseToDiscard("战烈：弃置一张牌，否则不可响应" + get.translation(trigger.card), "he").set("ai", (card) => {
  ...
  potzhenfeng: {
    ...
    enable: "phaseUse",
    filter(event, player) {
      return player.isDamaged() || ["pothanzhan", "potzhanlie"].some((skill2) => player.hasSkill(skill2, null, null, false));
    },
    skillAnimation: true,
      ...
      filter(button, player) {
        switch (button.link) {
          case "recover":
            return player.isDamaged();
          case "cover":
            return ["pothanzhan", "potzhanlie"].some((skill2) => player.hasSkill(skill2, null, null, false));
        }
      },
      check(button) {
        const player = get.player();
        ...
        return {
          ...
          async content(event, trigger, player) {
            ...
            if (get.info(event.name).item === "recover") {
              ...
              let dialog = [], skills2 = ["pothanzhan", "potzhanlie"].filter((skill2) => player.hasSkill(skill2, null, null, false)), list = [
                ["hp", "当前体力值"],
                ["damagedHp", "当前已损失体力值"],
                ["countplayer", "场上存活角色数"]
              ];
              dialog.push("振锋：修改" + skills2.map((skill2) => "〖" + get.translation(skill2) + "〗").join("和") + "描述中的“X”为...");
              for (const skill2 of skills2) {
                dialog.push('<div class="text center">' + get.translation(skill2) + "</div>");
                dialog.push([list.map((item) => [item[0] + "|" + skill2, item[1]]), "tdnodes"]);
              }
              ...
              if (result?.bool && result.links?.length) {
                ...
                for (const link of result.links) {
                  const [change, skill2] = link.split("|");
                  if (skill2 == "pothanzhan") {
                    changeList.push(change);
                  }
                  player.storage[skill2] = change;
                  player.popup(skill2);
                  game.log(player, "修改", "#g【" + get.translation(skill2) + "】", "的", "#yX", "为", "#g" + list.find((item) => item[0] === change)[1]);
                }
                if (changeList[0]) {
                  switch (changeList[0]) {
                    case "hp":
...
const translates = {
  ...
  potbihan: "蔽扞",
  potbihan_info: "一名角色受到【杀】的伤害时，你可令你或其将手牌弃至当前体力值，令此【杀】伤害-1。",
  pot_xiahouba: "势夏侯霸",
  ...
  potsifeng: "伺锋",
  potsifeng_info: "结束阶段，你可以将牌堆顶三张牌分配并扣置在任意名其他角色的武将牌上。其于回合内使用牌后，你移去其武将牌上的第一张“伺锋”牌，若此牌与移去的“伺锋”牌颜色不同，其弃置一张手牌；其回合结束时，你选择一项：1.获得其武将牌上的“伺锋”；2.移去其剩余的“伺锋”并对该角色造成1点伤害。",
  pot_lvyi: "势吕壹",
  pot_lvyi_prefix: "势",
  pothuilv: "隳律",
...
const dynamicTranslates = {
  potchiyun(player, skill2) {
    let s1 = player.storage[skill2 + "1"], s2 = player.storage[skill2 + "2"], t1 = "1.展示所有与这些牌颜色相同的手牌，你对其造成1点火焰伤害；", t2 = "2.你摸两张牌，其进入连环状态";
    if (s1) {
  ...
  potyanhui(player, skill2) {
    let s1 = player.storage[skill2 + "1"], s2 = player.storage[skill2 + "2"], t1 = "1.对一名因此弃置过牌的角色造成1点火焰伤害；", t2 = "2.摸X张牌（X为本回合展示过牌的角色数）";
    if (s1) {
      t1 = `<span style="text-decoration: line-through;">${t1}</span>`;
    }
    if (s2) {
  ...
  potfentao(player, skill2) {
    let s1 = player.storage[skill2 + "1"], s2 = player.storage[skill2 + "2"], t1 = "1.此次传导中的伤害+1；", t2 = "2.弃置一半牌（向上取整），此伤害结算后其进入连环状态";
    if (s1) {
      t1 = `<span style="text-decoration: line-through;">${t1}</span>`;
    }
    if (s2) {
  ...
  mbkechang(player, skill2) {
    if (player.getStorage(skill2, false)) {
      return lib.translate[`${skill2}_rewrite_info`];
    }
    return lib.translate[`${skill2}_info`];
  },
  potkuanggu(player) {
