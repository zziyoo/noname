...
const skills = {
  ...
  clanmingdian: {
    ...
    async content(event, trigger, player) {
      ...
      if (player.hasHistory("useCard", (evt) => names.includes(get.name(evt.card)))) {
        const list = player.getCards("h").map((card2) => get.name(card2)).unique();
  ...
  clanhuanjia: {
    ...
    async content(event, trigger, player) {
      ...
      let cards2 = [];
      let card;
      result = await player.chooseTarget(get.prompt("clanhuanjia"), "与一名其他角色拼点，赢的角色可以使用一张拼点牌。若此牌未造成过伤害，你获得另一张拼点牌，否则你失去一个技能", (card2, player2, target2) => {
...
const translates = {
  ...
  clanfenshi: "纷势",
  clanfenshi_info: "宗族技。每回合限一次，若有琅琊诸葛氏的手牌数等于场上势力数，则你使用的普通锦囊牌可额外结算一次。",
  clan_zhugezhan: "族诸葛瞻",
  clanchengwang: "承望",
