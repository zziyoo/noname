...
const skills = {
  ...
  sxrmxianger: {
    ...
    async content(event, trigger, player) {
      ...
      const map = player.getStorage(skill, /* @__PURE__ */ new Map());
      if (map.has(target)) {
    ...
    subSkill: {
      fail: {
        ...
        filter(event, player) {
          const num = (event.sxrmxiangerMap ?? /* @__PURE__ */ new Map()).get(player);
      ...
      mark: {
        intro: {
          content(storage = /* @__PURE__ */ new Map(), player) {
            if (!storage?.size) {
              return "目前没有【香饵】目标";
            } else {
      ...
      limit: {
        ...
        onremove(player, skill) {
          ...
          const targets = game.filterPlayer((target) => target.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map()).has(player));
          for (const target of targets) {
            const map = target.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map());
            map.delete(player);
        ...
        filter(event, player) {
          if (event.name === "damage") {
            return game.hasPlayer((target) => target.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map()).has(player));
          }
          return true;
        },
        forced: true,
        ...
        async content(event, trigger, player) {
          if (trigger.name === "damage") {
            for (const target of game.filterPlayer((target2) => target2.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map()).has(player))) {
              const map = target.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map());
              if (map.has(player)) {
            ...
            const sxrmxiangerMap = /* @__PURE__ */ new Map();
            for (const target of game.filterPlayer((target2) => target2.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map()).has(player)).sortBySeat()) {
              const num2 = target.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map()).get(player) || 0;
              sxrmxiangerMap.set(target, num2);
            }
      ...
      effect: {
        target(card, player, target) {
          if (get.tag(card, "damage")) {
            const targets = game.filterPlayer((current) => current.getStorage("sxrmxianger_mark", /* @__PURE__ */ new Map()).has(target));
  ...
  sxrmmieguo: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        filterTarget(card, player2, target) {
          return target !== player2 && target.hasCards("he");
        },
        ai(target) {
          const player2 = get.player();
          if (get.attitude(player2, target) > 0 && target.countCards("he") >= 4) {
            return 10;
          }
    ...
    async content(event, trigger, player) {
      ...
      result = game.countPlayer() <= num ? { bool: true, targets: game.filterPlayer() } : await target.chooseTarget({
        prompt: `${get.translation(player)}对你发动了【灭虢】`,
        prompt2: `请选择${get.cnNumber(num)}名角色，${get.translation(player)}的额外回合内无法对这些角色使用牌`,
        selectTarget: num,
        forced: true,
        ai(target2) {
          if (target2 === get.player()) {
            return 5;
          }
          return 1;
        }
  ...
  sxrmfusui: {
    ...
    filterTarget(card, player, target) {
      ...
      return target !== player && target.hasSex("male") && skills2.length > 0;
    },
    async content(event, trigger, player) {
      const target = event.targets[0];
      player.awakenSkill(event.name);
      const skills2 = target.getSkills(null, false, false).filter((skill2) => {
        const list = get.skillCategoriesOf(skill2, target);
        return !list.length;
      });
      if (!skills2.length) {
        return;
      ...
      const result = await player.chooseButton({
        createDialog: [`妇随：令${get.translation(target)}失去其中一个技能`, [skills2, "skill"]],
        forced: true
      }).forResult();
      ...
      for (const current of [player, target]) {
        ...
        await current.changeSkills(["sxrmbiyi"], []).set("$handle", (player2, addSkills, removeSkills) => {
          if (addSkills.length) {
            player2.addSkillLog(addSkills);
            if (addSkills.includes("sxrmbiyi")) {
              lib.skill.sxrmbiyi.init(player2, "sxrmbiyi");
            }
          }
        });
  ...
  sxrmbiyi: {
    ...
    init(player, skill) {
      ...
      const skills2 = !player.storage[skill] ? player.getStorage("sxrmbiyi_skill") : ["xiaoji"];
      if (skills2.length) {
        player.addAdditionalSkill(skill, skills2);
      }
    },
    onremove(player, skill) {
      player.removeAdditionalSkill(skill);
    ...
    intro: {
      content(storage, player) {
        if (!storage) {
          const skills2 = player.getStorage("sxrmbiyi_skill");
          if (skills2.length) {
            return `当前状态：阳（${skills2.map((skill) => get.poptip(skill)).join("、")}）`;
          }
          return `当前状态：阳（${skills2.length ? `${skills2.map((skill) => get.poptip(skill)).join("、")}` : "无技能"}）`;
        } else {
    ...
    filter(event, player) {
      ...
      const skills2 = !player.storage.sxrmbiyi ? player.getStorage("sxrmbiyi_skill") : ["xiaoji"];
  ...
  sxrmxiezhong: {
    ...
    async content(event, trigger, player) {
      ...
      for (const target2 of targets) {
        ...
        if (!lib.skill.sxrmxiezhong.canUseSha(target2)) {
          choiceMap.set(target2, 0);
        } else {
          let result2 = await target2.chooseControl({
            prompt: "挟众：请选择一项",
            ...
            ai() {
              ...
              return player2.getUseValue(get.autoViewAs({ name: "sha", isCard: true })) > 1 ? 1 : 0;
            }
          }).forResult();
          if (typeof result2.index == "number") {
            choiceMap.set(target2, result2.index);
          }
        }
      }
      if (!choiceMap.size) {
      ...
      for (const [target2, index] of choiceMap) {
        if (!target2.isIn()) {
          continue;
        }
        target2.popup(index === 0 ? "降" : "战");
        if (index == 0) {
      ...
      if (count1 === count2 || !game.hasPlayer((target2) => !targets.includes(target2))) {
        return;
      }
      ...
      const result = await player.chooseTarget({
        ...
        ai(target2) {
          ...
          const { num: num2 } = get.event();
          let val;
          if (att > 0) {
            val = 1;
            if (target2.hp >= 6 && num2 === 0) {
              val = 3;
            }
            if (target2.countCards("he") === 1 && num2 === 1) {
              val = 6;
            }
          } else {
            val = 2;
            if (target2.hp <= 1 && num2 === 0) {
              val = 5;
            } else if (target2.countCards("he") <= 4 && target2.countCards("he") > 1 && num2 === 1) {
              val = 4;
            }
            if (target2.countCards("he") === 1 && num2 === 1) {
              val = 0.5;
    ...
    canUseSha(player) {
      let cards2 = player.getCards("hes");
      if (cards2.length < 2) {
        return false;
      }
      for (let i = 0; i < cards2.length - 1; i++) {
        for (let j = i + 1; j < cards2.length; j++) {
          const sha = get.autoViewAs({ name: "sha" }, [cards2[i], cards2[j]]);
  ...
  sxrmqishi: {
      ...
      game.checkGlobalHistory("cardMove", (evt) => {
        if (evt.name == "lose" && evt.player != player && evt.position === ui.discardPile) {
          ...
          for (const current of game.filterPlayer()) {
            if (current != player) {
              cards2.addArray(evt.getd(current, "cards2"));
            }
          }
        }
      });
      cards2 = cards2.filterInD("d").flat().unique();
      return cards2;
    },
    filter(event, player) {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player.chooseButton({
        createDialog: [`${!forced ? get.prompt2(event.skill) : "###乞施###<div class='text center'>获得本回合其他角色进入弃牌堆的至多五张牌，然后你跳过下个摸牌阶段</div>"}`, cards2],
        selectButton: [1, 5],
        allowChooseAll: true,
        forced,
        ai(button) {
          const { cards: cards3 } = get.event();
          if (cards3.length < 4) {
            return -1;
          }
  ...
  sxrmwanli: {
    ...
    async content(event, trigger, player) {
      ...
      const map = player.getStorage(skill, /* @__PURE__ */ new Map());
      if (map.has(target)) {
        map.get(target).giveNum += 3 * cards2.length;
      } else {
        map.set(target, {
          giveNum: 3 * cards2.length,
          roundNumber: 4,
          huanqian: false
        });
      }
    ...
    subSkill: {
      effect: {
        ...
        onremove: true,
        intro: {
          content(storage = /* @__PURE__ */ new Map()) {
            if (!storage?.size) {
              return "账单是空的？！";
            }
            let str = "当前账单：<br>";
        ...
        logTarget(event, player) {
          return game.filterPlayer((current) => {
            const storage = player.getStorage("sxrmwanli_effect", /* @__PURE__ */ new Map());
            return storage.has(current) && !storage.get(current).huanqian && game.roundNumber == storage.get(current).roundNumber;
          }).sortBySeat();
        },
        async content(event, trigger, player) {
          const storage = player.getStorage(event.name, /* @__PURE__ */ new Map());
          for (const target of event.targets) {
            if (!target.isIn() || !storage.has(target)) {
              continue;
            }
            const num = storage.get(target).giveNum;
            const map = player.getStorage(event.name, /* @__PURE__ */ new Map());
      ...
      draw: {
        ...
        filter(event, player) {
          return !event.numFixed && game.hasPlayer2((current) => {
            const storage = player.getStorage("sxrmwanli_effect", /* @__PURE__ */ new Map());
        ...
        locked: false,
        async content(event, trigger, player) {
          trigger.num += 3 * game.countPlayer2((current) => {
            const storage = player.getStorage("sxrmwanli_effect", /* @__PURE__ */ new Map());
            if (!storage.has(current)) {
              return false;
            }
  ...
  sxrmlishui: {
    ...
    filter(event, player) {
      return get.color(event.card) == "black" && game.hasPlayer((current) => {
        const storage = player.getStorage("sxrmwanli_effect", /* @__PURE__ */ new Map());
    ...
    async content(event, trigger, player) {
      ...
      let result = targets.length == 1 ? { bool: true, targets } : await player.chooseTarget({
        prompt: "选择一名“万利”角色拼点",
        filterTarget: (card, player2, target2) => {
      ...
      if (result?.winner == player) {
        const map = player.getStorage("sxrmwanli_effect", /* @__PURE__ */ new Map());
        if (!map.size) {
          return;
        }
        player.say("还钱！");
  ...
  sxrmlanjiao: {
    ...
    async content(event, trigger, player) {
      ...
      event.showCards ??= [];
      while (true) {
        let result = await player.choosePlayerCard({
        ...
        if (result?.bool) {
          ...
          event.showCards = cards2;
          await player.showCards(cards2, `${get.translation(player)}发动了【揽娇】`);
          result = await target.chooseBool({
            ai() {
              const { player: player2, cards: cards3 } = get.event();
              return (player2.hp > 3 || get.effect(player2, { name: "losehp" }, player2, player2) > 4) && !cards3.some((card) => get.color(card) == "red");
            },
            cards: cards2
          }).set("createDialog", [`###${get.translation(player)}对你发动了【揽娇】###你可以失去1点体力并摸一张牌，令${get.translation(player)}重新选择展示的手牌`, cards2]).forResult();
          if (!result?.bool) {
            break;
          } else {
            await target.draw();
      ...
      if (get.itemtype(event.showCards) == "cards") {
        ...
        const suits2 = event.showCards.map((card) => get.suit(card, false)).toUniqued();
        const skill = event.name + "_mark";
        player.addTempSkill(skill);
        player.markAuto(skill, suits2);
        player.addTip(
          skill,
          get.translation(skill) + player.getStorage(skill).sort((a, b) => lib.suit.indexOf(b) - lib.suit.indexOf(a)).reduce((str, suit) => str + get.translation(suit), "")
        );
      }
  ...
  sxrmbingqu: {
    ...
    hasUseTarget(player, card) {
      ...
      function* combinations(cards2, chooseCount) {
        ...
        function* dfs(start, depth) {
          ...
          const limit = cards2.length - (chooseCount - depth);
          for (let i = start; i <= limit; i++) {
            choose[depth] = cards2[i];
            yield* dfs(i + 1, depth + 1);
          }
        }
        yield* dfs(0, 0);
      }
      for (const cards2 of combinations(hs, count)) {
        if (player.hasUseTarget(get.autoViewAs(card, cards2))) {
          return true;
        }
      }
      return false;
    ...
    async content(event, trigger, player) {
      ...
      for (const current of [player, target]) {
        ...
        const result = await current.chooseButton({
          createDialog: [`###请声明一个锦囊牌名###<div class='text center'>${get.translation(targetx)}将选择其半数手牌当此牌使用</div>`, [vcards, "vcard"]],
          forced: true,
          ai(button) {
            const { player: player2, target: target2 } = get.event();
            return -target2.getUseValue({ name: button.link[2], nature: button.link[3] });
          }
        }).set("target", targetx).forResult();
        if (result?.bool) {
      ...
      for (const current of [player, target]) {
        ...
        game.broadcastAll(
          (viewAs, num2) => {
            ...
            lib.skill.sxrmbingqu_backup.selectCard = num2;
          },
          card,
          num
        );
  ...
  sxrmfanxin: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        filterTarget: (card, player2, target) => {
          return target != player2 && !player2.getStorage("sxrmfanxin").includes(target);
        },
        ai(target) {
          const player2 = get.player();
          return -get.attitude(player2, target);
    ...
    subSkill: {
      draw: {
        ...
        async cost(event, trigger, player) {
          const target = trigger.player, num = Math.min(5, target.countMark("baonu"));
  ...
  sxrmfubei: {
    ...
    async content(event, trigger, player) {
      ...
      result = await player.chooseNumbers({
        ...
        filterOk(event2) {
          return event2.numbers.toUniqued().length == 2;
        },
        processAI() {
      ...
      }).set("range0", insertRange).forResult();
      ...
      player.setStorage("sxrmfubei_effect", player.getStorage("sxrmfubei_effect").concat(cards2));
      game.broadcastAll(function() {
        lib.skill.sxrmfubei.createObserver();
      });
      await player.lose(cards2, ui.cardPile).set("insert_index", (event2, card) => {
        const nums = get.event().nums, cardsx = get.event().cardsx;
        return ui.cardPile.childNodes[nums[cardsx.indexOf(card)] - 1];
      }).set("nums", [num1, num2]).set("cardsx", [card1, card2]);
    ...
    // 【知天】看牌
    clickable(player) {
      const cards2 = lib.skill.sxrmfubei.getCards(player);
    ...
    getCards(player) {
      let cards2 = [];
      if (game.online) {
        return game.requestSkillData("sxrmfubei", "getTopCards", 1e4);
      } else {
        if (ui.cardPile.hasChildNodes !== false) {
          cards2 = Array.from(ui.cardPile.childNodes).slice(0, 10);
        }
      }
      return cards2;
    },
        ...
        for (const mutation of mutationsList) {
          if (mutation.type === "childList") {
            ...
            mutation.removedNodes.forEach((card) => {
              ...
              game.broadcastAll((card2) => {
                if (card2?.gaintag?.length) {
                  card2.removeGaintag("visible_sxrmfubei");
                }
              }, card);
            });
          }
  ...
  sxrmdancui: {
    ...
    async cost(event, trigger, player) {
      const qiding = player.storage["sxrmdancui"], target = trigger.player;
      if (player.countDiscardableCards(player, "he") < 2) {
        ...
        if (qiding) {
          ...
          const result = await player.chooseBool({
            prompt: get.prompt(event.skill, target),
            prompt2: `${cards2.length ? `弃置${get.translation(cards2)}，` : ""}使${get.translation(target)}受到的伤害值+1？`,
            ai() {
              const { player: player2, target: target2 } = get.event();
  ...
  sxrmzhaduo: {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseTarget({
        ...
        selectTarget: 2,
        filterTarget(card, player2, target) {
          return player2 != target && target.hasGainableCards(player2, "he");
        },
        ai(target) {
          const player2 = get.player();
          const eff1 = get.effect(target, { name: "shunshou_copy2" }, player2, player2);
          if (!ui.selected.targets.length) {
            return eff1 + get.effect(target, { name: "sha" }, player2, player2) + target.getSkills(null, false, false).filter((skill) => {
              const info = get.info(skill);
              return info?.ai && (info.ai.maixie || info.ai.maixie_hp || info.ai.maixie_defend);
            }).length * 3;
          }
          const target1 = ui.selected.targets[0];
          return eff1 + get.effect(player2, { name: "juedou" }, target, player2) + target1.getSkills(null, false, false).filter((skill) => {
            const info = get.info(skill);
            return info?.ai && (info.ai.maixie || info.ai.maixie_hp || info.ai.maixie_defend);
          }).length * 3;
        }
      }).set("targetprompt", ["出杀对象", "决斗对象"]).set("multitarget", true).forResult();
    },
    async content(event, trigger, player) {
      ...
      if (player.canUse(sha, target1, false)) {
        ...
        if (target2.canUse(juedou, player, false)) {
          const skills12 = target1.getSkills(null, false, false);
          player.addAdditionalSkill(event.name, skills12);
  ...
  sxrmyangbei: {
    ...
    filter(event, player, name) {
      ...
      if (hs.some((card2) => !lib.filter.cardDiscardable(card2, player, "sxrmyangbei"))) {
        return false;
      }
      return true;
    },
      ...
      if (event.triggername != "useCardToPlayered") {
        ...
        event.result = await target.chooseBool({
          ...
          ai() {
            ...
            switch (player2.countCards("h")) {
              case 1:
                return true;
              case 2:
              case 3:
                return player2.countCards("h", (card2) => {
                  return ["shan", "tao"].includes(card2.name);
                }) <= 1;
              default:
                return false;
            }
  ...
  sxrmsigu: {
    ...
    async content(event, trigger, player) {
      ...
      if (name && skill) {
        game.broadcastAll((player2, name2) => player2.tempname.add(name2), target, "sxrm_caocao");
      ...
      if (name && skill) {
        if (Array.isArray(target.tempname)) {
          game.broadcastAll((player2, name2) => player2.tempname.remove(name2), target, "sxrm_caocao");
        }
...
const dynamicTranslates = {
  ...
  sxrmbiyi(player, skill) {
    ...
    const skills2 = player.getStorage("sxrmbiyi_skill");
    let yang = skills2.length ? `${skills2.map((skill2) => get.poptip(skill2)).join("、")}` : "无技能", yin = `${get.poptip("xiaoji")}`;
    if (storage) {
      yin = `<span class='bluetext'>${yin}</span>`;
    } else {
      yang = `<span class='firetext'>${yang}</span>`;
    }
    let start = "转换技。你视为拥有：", end = "。当你发动当前分支的对应技能时，切换本技能状态。";
