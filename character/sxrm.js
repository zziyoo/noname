...
const skills = {
    ...
    async cost(event, trigger, player) {
      event.result = await player.chooseBool({
        ...
        ai() {
          ...
          return player2.hasCards("h", (card) => get.tag(card, "damage") && player2.getUseValue(card));
        }
      }).forResult();
    },
      ...
      limit: {
        ...
        async content(event, trigger, player) {
          if (trigger.name === "damage") {
            for (const target of game.filterPlayer((target2) => target2.hasSkill("sxrmxianger") && target2.getStorage("sxrmxianger_mark")?.some((i) => i.target === player))) {
            ...
            for (const target of targets.sortBySeat()) {
              const item = target.storage.sxrmxianger_mark.find((i) => i.target === player);
              const num2 = item.damage;
              list.push([target, num2]);
            }
            player.removeSkill(event.name);
            await player.recover({ num });
            for (const [target, num2] of list) {
              if (num2 < 2) {
                target.line(player);
  ...
  sxrmmieguo: {
    ...
    async content(event, trigger, player) {
      ...
      const targetsx = game.countPlayer() <= num ? game.filterPlayer() : (await target.chooseTarget({
