...
export default {
	...
	card: {
				...
				if (result.index === 0) {
					...
					if (list.length > 1) {
						result2 = await target
							.chooseToDiscard("h", true, "请弃置两张类型不同的手牌", 2, (card, player) => {
		...
		liulongcanjia: {
			...
			ai: {
				equipValue(card, player) {
					...
					if (
						game.hasPlayer(current => current.hasSkill("gzzongyu") && get.attitude(player, current) <= 0)
					) {
						return 1;
					}
					return 7.2;
		...
		dinglanyemingzhu: {
			...
			ai: {
				equipValue(card, player) {
					...
					if (
						game.hasPlayer(current => current.hasSkill("jubao") && get.attitude(player, current) <= 0)
					) {
						return 0;
					}
					return 7;
		...
		feilongduofeng: {
			...
			ai: {
				equipValue(card, player) {
					...
					if (
						game.hasPlayer(current => current.hasSkill("zhangwu") && get.attitude(player, current) <= 0)
					) {
						return 1;
					}
					return 8;
		...
		taipingyaoshu: {
			...
			ai: {
				equipValue(card, player) {
					...
					if (
						game.hasPlayer(current => current.hasSkill("wendao") && get.attitude(player, current) <= 0)
					) {
						return 1;
					}
					return 6;
		...
		jilinqianyi: {
			...
			ai: {
				equipValue(card, player) {
					...
					if (
						game.hasPlayer(current => current.hasSkill("gz_shujuan") && get.attitude(player, current) <= 0 && player.getDamagedHp())
					) {
						return 1;
					}
					return 4;
			...
			async content(event, trigger, player) {
				...
				const result = await target
					.chooseControl({
						...
						ai(event, player) {
							const eff = get.damageEffect(player, event.player, player, "thunder");
							if (eff > 0) {
								return "take_damage";
							}
			...
			ai: {
				...
				yingbian(card, player, targets, viewer) {
					...
					if (get.cardtag(card, "yingbian_all")) {
						if (
							targets.filter(current => {
								return get.damageEffect(current, player, player, "thunder") > 0 && current.countCards("e", card => get.value(card, current) <= 0) < 2 && current.countCards("e", card => get.value(card, current) > 0) > 0;
							}).length
						) {
							base += 6;
						}
		...
		huoshaolianying: {
			...
			filterTarget(card, player, target) {
				...
				if (
					game.hasPlayer(current => current.isLinked() && current !== player)
				) {
					if (!target.isLinked()) {
						return false;
					}
					...
					return !game.hasPlayer(current => {
						if (target !== current && current !== player && current.isLinked()) {
							const dist = get.distance(player, current, "absolute");
							if (dist < distance) {
								return true;
							}
		...
		wuliu: {
			...
			ai: {
				equipValue(card, player) {
					...
					return (
						2 +
						game.countPlayer(current => current.isFriendOf(player)) /
						2
					);
				},
				basic: {
	...
	skill: {
		...
		zhaoshu_global: {
			...
			selectTarget() {
				if (
					game.countPlayer(current => current.hasSkill("zhaoshu_skill") && current.isFriendOf(_status.event.player)) === 1
				) {
					return -1;
				}
				return 1;
			...
			async content(event, trigger, player) {
				...
				const result = await player
					.chooseCardTarget({
						...
						filterTarget(card, player, target) {
							const damaged = _status.event.damaged;
